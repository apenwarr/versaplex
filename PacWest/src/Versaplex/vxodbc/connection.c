...
static int md5_auth_send(ConnectionClass * self, const char *salt)
{
...
static char
LIBPQ_CC_connect(ConnectionClass * self, char password_req,
		 char *salt_para)
{
    int ret;
...
static char
original_CC_connect(ConnectionClass * self, char password_req,
		    char *salt_para)
{
    StartupPacket sp;
	...
	{
	    CC_set_error(self, CONNECTION_SERVER_NOT_REACHED,
			 "Could not connect to the server", func);
	    return 0;
	}
	...
	if (PROTOCOL_62(ci))
	{
	    ...
	    SOCK_put_n_char(sock, (char *) &sp62,
			    sizeof(StartupPacket6_2));
	    SOCK_flush_output(sock);
	...
	if (SOCK_get_errcode(sock) != 0)
	{
	    CC_set_error(self, CONN_INVALID_AUTHENTICATION,
			 "Failed to send the authentication packet",
			 func);
	    return 0;
	}
	...
	do
	{
		...
		if (PROTOCOL_74(ci))
		{
			...
			strncpy(ci->protocol, PG74REJECTED,
				sizeof(ci->protocol));
/* retry = TRUE; */
		}
		startPacketReceived = TRUE;
		...
		handle_error_message(self, msgbuffer, sizeof(msgbuffer),
				     self->sqlstate, func, NULL);
		CC_set_error(self, CONN_INVALID_AUTHENTICATION,
			     msgbuffer, func);
		qlog("ERROR from backend during authentication: '%s'\n",
		     msgbuffer);
		if (strnicmp
		    (msgbuffer, "Unsupported frontend protocol",
		     29) == 0)
		    retry = TRUE;
		else if (strncmp(msgbuffer, "FATAL:", 6) == 0 &&
			 strnicmp(msgbuffer + 8,
		...
		if (password_req != AUTH_REQ_OK)
		{
		    ...
		    mylog("salt=%02x%02x%02x%02x%02x\n",
			  (UCHAR) salt[0], (UCHAR) salt[1],
			  (UCHAR) salt[2], (UCHAR) salt[3],
			  (UCHAR) salt[4]);
		} else
		{
		...
		case AUTH_REQ_KRB4:
		    CC_set_error(self, CONN_AUTH_TYPE_UNSUPPORTED,
		...
		case AUTH_REQ_PASSWORD:
		    ...
		    if (ci->password[0] == '\0')
		    {
			CC_set_error(self, CONNECTION_NEED_PASSWORD,
			...
			SOCK_put_char(sock, 'p');
		    SOCK_put_int(sock,
				 (Int4) (4 + strlen(ci->password) + 1),
				 4);
		    SOCK_put_n_char(sock, ci->password,
				    (Int4) strlen(ci->password) + 1);
		...
		default:
		    CC_set_error(self, CONN_AUTH_TYPE_UNSUPPORTED,
				 "Unknown authentication type", func);
		    return 0;
		}
		break;
		...
		handle_notice_message(self, notice, sizeof(notice),
				      self->sqlstate, "CC_connect",
				      NULL);
		break;
	    default:
		snprintf(notice, sizeof(notice),
			 "Unexpected protocol character='%c' during authentication",
			 beresp);
		CC_set_error(self, CONN_INVALID_AUTHENTICATION, notice,
			     func);
		return 0;
	    }
	    if (retry)
  ...
  sockerr_proc:
	...
	{
	    CC_set_error(self, CONNECTION_NO_SUCH_DATABASE,
...
CC_connect(ConnectionClass * self, char password_req, char *salt_para)
{
	...
	if (0 == ret
	    && CONN_AUTH_TYPE_UNSUPPORTED == CC_get_errornumber(self))
	{
	    SOCK_Destructor(self->sock);
	    self->sock = (SocketClass *) 0;
	    ret = LIBPQ_CC_connect(self, password_req, salt_para);
	...
	{
	    if (!self->original_client_encoding || UTF8 != self->ccsc)
	    {
		QResultClass *res;
		if (PG_VERSION_LT(self, 7.1))
		{
		    CC_set_error(self, CONN_NOT_IMPLEMENTED_ERROR,
				 "UTF-8 conversion isn't implemented before 7.1",
				 func);
		    return 0;
		}
		if (self->original_client_encoding)
		...
		self->original_client_encoding = NULL;
		if (res =
		    CC_send_query(self, "set client_encoding to 'UTF8'",
				  NULL, 0, NULL),
    ...
    {
	CC_set_error(self, CONN_NOT_IMPLEMENTED_ERROR,
		     "Unicode isn't supported before 6.4", func);
	return 0;
    }
#endif				/* UNICODE_SUPPORT */
    ...
    if (CC_is_in_unicode_driver(self) && 0 < ci->bde_environment)
			    ...
			    StatementClass * stmt)
{
	...
	issue_begin = ((flag & GO_INTO_TRANSACTION) != 0
		       && !CC_is_in_trans(self)), rollback_on_error,
	query_rollback;
	...
	discard_next_savepoint = FALSE, consider_rollback;
	...
	CC_set_error(self, CONNECTION_COULD_NOT_SEND,
		     "Could not send Query(connection dead)", func);
    ...
    {
	CC_set_error(self, CONNECTION_MSG_TOO_LONG,
		     "Query string is too long", func);
	return NULL;
    }
    ...
    {
	CC_set_error(self, CONNECTION_COULD_NOT_SEND,
		     "Could not send Query to backend", func);
	CC_on_abort(self, CONN_DEAD);
	return NULL;
    }
...
#define	return DONT_CALL_RETURN_FROM_HERE???
	...
	if (!cmdres)
	{
	    CC_set_error(self, CONNECTION_COULD_NOT_RECEIVE,
	...
	{
	    CC_set_error(self, CONNECTION_NO_RESPONSE,
			 "No response from the backend", func);
	...
	case 'C':		/* portal query command, no tuples
	    ...
	    {
		CC_set_error(self, CONNECTION_NO_RESPONSE,
			     "No response from backend while receiving a portal query command",
			     func);
	...
	case 'N':		/* NOTICE: */
	    msg_truncated =
		handle_notice_message(self, cmdbuffer,
				      sizeof(cmdbuffer), res->sqlstate,
				      "send_query", res);
	...
	case 'I':		/* The server sends an empty query */
		...
		QR_set_message(res,
			       "Unexpected protocol character from backend (send_query - I)");
		QR_set_rstatus(res, PORES_FATAL_ERROR);
		ReadyToReturn = TRUE;
		retres = cmdres;
		break;
  ...
  cleanup:
	...
	if (0 == CC_get_errornumber(self))
	    CC_set_error(self, CONNECTION_COMMUNICATION_ERROR,
			 "Communication error while sending query",
			 func);
	CC_on_abort(self, CONN_DEAD);
	ReadyToReturn = TRUE;
	retres = NULL;
	...
	if (query_rollback)
	{
		...
		snprintf(cmd, sizeof(cmd), "ROLLBACK TO %s;",
			 per_query_svp);
	    snprintf_add(cmd, sizeof(cmd), "RELEASE %s", per_query_svp);
	} else if (CC_is_in_error_trans(self))
	    strcpy(cmd, "ROLLBACK");
	if (cmd[0])
	    QR_Destructor(CC_send_query
			  (self, cmd, NULL, IGNORE_ABORT_ON_CONN,
			   NULL));
    }
...
static void CC_lookup_lo(ConnectionClass * self)
{
	...
	res =
	    CC_send_query(self,
			  ...
			  PG_TYPE_LO_NAME "'", NULL,
			  IGNORE_ABORT_ON_CONN | ROLLBACK_ON_ERROR,
			  NULL);
    else
	res =
	    CC_send_query(self,
			  "select oid, 0 from pg_type where typname='"
			  PG_TYPE_LO_NAME "'", NULL,
			  IGNORE_ABORT_ON_CONN | ROLLBACK_ON_ERROR,
			  NULL);
    if (QR_command_maybe_successful(res)
	&& QR_get_num_cached_tuples(res) > 0)
    {
	Oid basetype;
