...
UCHAR *check_client_encoding(const UCHAR * conn_settings)
{
	...
	case 3:
	    ...
	    {
		cptr++;
		for (sptr = cptr; *cptr && *cptr != LITERAL_QUOTE;
		     cptr++);
	    } else
	    {
...
const UCHAR *pg_CS_name(int characterset_code)
{
    int i;
    for (i = 0; CS_Table[i].code != OTHER; i++)
    {
...
int pg_CS_stat(int stat, unsigned int character, int characterset_code)
{
    if (character == 0)
	stat = 0;
    switch (characterset_code)
...
/* EUC_JP Support */
    case EUC_JP:
	{
		...
		stat = 3;
	    else if (stat != 2 && (character == 0x8e || character > 0xa0))	/* Half Katakana HighByte & Kanji HighByte */
		stat = 2;
	    else if (stat == 2)
		stat = 1;
	    else
...
UCHAR *pg_mbschr(int csc, const UCHAR * string, unsigned int character)
{
    int mb_st = 0;
...
size_t pg_mbslen(int csc, const UCHAR * string)
{
    UCHAR *s;
    size_t len;
    int cs_stat;
...
UCHAR *pg_mbsinc(int csc, const UCHAR * current)
{
    int mb_stat = 0;
    if (*current != 0)
    {
...
static char *CC_lookup_cs_new(ConnectionClass * self)
{
    char *encstr = NULL;
    QResultClass *res;
...
static char *CC_lookup_cs_old(ConnectionClass * self)
{
    char *encstr = NULL;
    HSTMT hstmt;
    RETCODE result;
 ...
 */
const char *get_environment_encoding(const ConnectionClass * conn,
				     const char *setenc,
				     const char *currenc, BOOL bStartup)
{
    ...
    {
	if (bStartup || stricmp(currenc, "SQL_ASCII") == 0)
	    return wenc;
    }
    switch (acp)
    {
...
void CC_lookup_characterset(ConnectionClass * self)
{
	...
	if (stricmp(self->original_client_encoding, tencstr))
	{
	    ...
	    snprintf(msg, sizeof(msg),
		     "The client_encoding '%s' was changed to '%s'",
    ...
    {
	const char *wenc =
	    get_environment_encoding(self, encspec, currenc, FALSE);
	...
	if (self->ccsc < 0)
	{
	    ...
	    snprintf(msg, sizeof(msg),
		     "would handle the encoding '%s' like ASCII",
		     tencstr);
	    CC_set_error(self, CONN_OPTION_VALUE_CHANGED, msg, func);
	}
    } else
...
int encoded_nextchar(encoded_str * encstr)
{
    ...
    encstr->ccst =
	pg_CS_stat(encstr->ccst, (unsigned int) chr, encstr->ccsc);
    return chr;
}
