...
static void parameter_bindings_set(APDFields * opts, int params,
				   BOOL maxset)
{
    int i;
...
static void parameter_ibindings_set(IPDFields * opts, int params,
				    BOOL maxset)
{
    int i;
	    ...
	    SQLINTEGER BufferLength)
{
	...
	return ret;
    case SQL_DESC_COUNT:
	parameter_bindings_set(opts, CAST_PTR(SQLSMALLINT, Value),
			       FALSE);
	return ret;
	    ...
	    SQLINTEGER BufferLength, SQLINTEGER * StringLength)
{
	...
	rettype = SQL_IS_SMALLINT;
	ival =
	    NAME_IS_NULL(ipdopts->parameters[para_idx].
			 paramName) ? SQL_UNNAMED : SQL_NAMED;
	break;
    case SQL_DESC_TYPE:
	rettype = SQL_IS_SMALLINT;
	switch (ipdopts->parameters[para_idx].SQLType)
		   ...
		   SQLINTEGER * StringLength)
{
	...
	ret =
	    APDGetField(desc, RecNumber, FieldIdentifier, Value,
			BufferLength, StringLength);
	break;
    case SQL_ATTR_IMP_ROW_DESC:
	ret =
	    IRDGetField(desc, RecNumber, FieldIdentifier, Value,
			BufferLength, StringLength);
	break;
    case SQL_ATTR_IMP_PARAM_DESC:
	ret =
	    IPDGetField(desc, RecNumber, FieldIdentifier, Value,
			BufferLength, StringLength);
	break;
    default:
	ret = SQL_ERROR;
		   ...
		   PTR Value, SQLINTEGER BufferLength)
{
    ...
    mylog("%s h=%p(%d) rec=%d field=%d val=%p,%d\n", func,
	  DescriptorHandle, desc->desc_type, RecNumber, FieldIdentifier,
	  Value, BufferLength);
    switch (desc->desc_type)
    {
    case SQL_ATTR_APP_ROW_DESC:
    ...
    case SQL_ATTR_IMP_ROW_DESC:
	ret =
	    IRDSetField(desc, RecNumber, FieldIdentifier, Value,
			BufferLength);
	break;
    case SQL_ATTR_IMP_PARAM_DESC:
	ret =
	    IPDSetField(desc, RecNumber, FieldIdentifier, Value,
			BufferLength);
	break;
    default:
	ret = SQL_ERROR;
	...
	if (!DC_get_errormsg(desc))
	{
	    ...
	    case DESC_INVALID_COLUMN_NUMBER_ERROR:
		DC_set_errormsg(desc,
				"can't SQLSetDescField for this column number");
		break;
	    case DESC_BAD_PARAMETER_NUMBER_ERROR:
		DC_set_errormsg(desc,
