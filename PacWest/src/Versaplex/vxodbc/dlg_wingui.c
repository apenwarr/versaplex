...
static int driver_optionsDraw(HWND, const ConnInfo *, int src,
			      BOOL enable);
...
void GetDlgStuff(HWND hdlg, ConnInfo * ci)
{
    ...
    sslposition =
	(int) (DWORD) SendMessage(GetDlgItem(hdlg, IDC_SSLMODE),
				  CB_GETCURSEL, 0L, 0L);
    switch (sslposition)
    {
    case 1:
	strncpy(ci->sslmode, "prefer", sizeof(ci->sslmode));
	break;
    case 2:
	strncpy(ci->sslmode, "allow", sizeof(ci->sslmode));
	break;
    case 3:
	strncpy(ci->sslmode, "require", sizeof(ci->sslmode));
	break;
    default:
	strncpy(ci->sslmode, "disable", sizeof(ci->sslmode));
	break;
    }
}
...
driver_optionsDraw(HWND hdlg, const ConnInfo * ci, int src, BOOL enable)
{
	...
	if (!defset)
	{
	    ...
	    defval.unknowns_as_longvarchar =
    ...
    ShowWindow(GetDlgItem(hdlg, DRV_MSG_LABEL2),
    ...
    CheckDlgButton(hdlg, DRV_TEXT_LONGVARCHAR,
		   comval->text_as_longvarchar);
    CheckDlgButton(hdlg, DRV_UNKNOWNS_LONGVARCHAR,
    ...
    CheckDlgButton(hdlg, DRV_CANCELASFREESTMT,
		   comval->cancel_as_freestmt);
    ...
    SetDlgItemInt(hdlg, DRV_VARCHAR_SIZE, comval->max_varchar_size,
		  FALSE);
    SetDlgItemInt(hdlg, DRV_LONGVARCHAR_SIZE,
		  comval->max_longvarchar_size, TRUE);
    SetDlgItemText(hdlg, DRV_EXTRASYSTABLEPREFIXES,
    ...
    ShowWindow(GetDlgItem(hdlg, IDPREVPAGE),
	       enable ? SW_HIDE : SW_SHOW);
    ShowWindow(GetDlgItem(hdlg, IDNEXTPAGE),
	       enable ? SW_HIDE : SW_SHOW);
    return 0;
}
		      ...
		      const char *updateDriver)
{
    ...
    comval->use_declarefetch =
	IsDlgButtonChecked(hdlg, DRV_USEDECLAREFETCH);
    ...
    comval->unknowns_as_longvarchar =
	IsDlgButtonChecked(hdlg, DRV_UNKNOWNS_LONGVARCHAR);
    comval->bools_as_char = IsDlgButtonChecked(hdlg, DRV_BOOLS_CHAR);

    comval->parse = IsDlgButtonChecked(hdlg, DRV_PARSE);

    comval->cancel_as_freestmt =
	IsDlgButtonChecked(hdlg, DRV_CANCELASFREESTMT);
    comval->debug = IsDlgButtonChecked(hdlg, DRV_DEBUG);

    comval->fetch_max =
	GetDlgItemInt(hdlg, DRV_CACHE_SIZE, NULL, FALSE);
    comval->max_varchar_size =
	GetDlgItemInt(hdlg, DRV_VARCHAR_SIZE, NULL, FALSE);
    comval->max_longvarchar_size = GetDlgItemInt(hdlg, DRV_LONGVARCHAR_SIZE, NULL, TRUE);	/* allows for
												 * SQL_NO_TOTAL */

    GetDlgItemText(hdlg, DRV_EXTRASYSTABLEPREFIXES,
		   comval->extra_systable_prefixes,
		   sizeof(comval->extra_systable_prefixes));

    /* Driver Connection Settings */
    if (!ci)
	GetDlgItemText(hdlg, DRV_CONNSETTINGS, comval->conn_settings,
		       sizeof(comval->conn_settings));

    if (updateDriver)
    {
	if (writeDriverCommoninfo(ODBCINST_INI, updateDriver, comval) <
	    0)
	    MessageBox(hdlg, "impossible to update the values, sorry",
...
LRESULT CALLBACK
driver_optionsProc(HWND hdlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    ConnInfo *ci;
    char strbuf[128];
	...
	ci = (ConnInfo *) lParam;
	LoadString(s_hModule, IDS_ADVANCE_OPTION_DEF, strbuf,
		   sizeof(strbuf));
...
LRESULT CALLBACK
ds_options1Proc(HWND hdlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    ConnInfo *ci;
    char strbuf[128];
	...
	case IDAPPLY:
	    driver_options_update(hdlg, ci, NULL);
	    SendMessage(GetWindow(hdlg, GW_OWNER), WM_COMMAND, wParam,
			lParam);
	    break;
...
LRESULT CALLBACK
ds_options2Proc(HWND hdlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    ConnInfo *ci;
    char buf[128];
    DWORD cmd;
	...
	{
	    LoadString(s_hModule, IDS_ADVANCE_OPTION_CON2, buf,
		       sizeof(buf));
	    SetWindowText(hdlg, buf);
	    ShowWindow(GetDlgItem(hdlg, IDAPPLY), SW_HIDE);
	}
	...
	CheckDlgButton(hdlg, DS_SHOWOIDCOLUMN,
		       atoi(ci->show_oid_column));
	CheckDlgButton(hdlg, DS_FAKEOIDINDEX, atoi(ci->fake_oid_index));
	CheckDlgButton(hdlg, DS_ROWVERSIONING,
		       atoi(ci->row_versioning));
	CheckDlgButton(hdlg, DS_SHOWSYSTEMTABLES,
		       atoi(ci->show_system_tables));
	CheckDlgButton(hdlg, DS_DISALLOWPREMATURE,
	...
	CheckDlgButton(hdlg, DS_SERVERSIDEPREPARE,
		       ci->use_server_side_prepare);
	CheckDlgButton(hdlg, DS_BYTEAASLONGVARBINARY,
		       ci->bytea_as_longvarbinary);
	/*CheckDlgButton(hdlg, DS_LOWERCASEIDENTIFIER, ci->lower_case_identifier); */

	EnableWindow(GetDlgItem(hdlg, DS_FAKEOIDINDEX),
		     atoi(ci->show_oid_column));
	...
	case DS_SHOWOIDCOLUMN:
	    ...
	    EnableWindow(GetDlgItem(hdlg, DS_FAKEOIDINDEX),
			 IsDlgButtonChecked(hdlg, DS_SHOWOIDCOLUMN));
	    return TRUE;
	...
	case IDPREVPAGE:
	    ...
	    /* Readonly */
	    sprintf(ci->onlyread, "%d",
		    IsDlgButtonChecked(hdlg, DS_READONLY));
	    ...
	    sprintf(ci->show_system_tables, "%d",
		    IsDlgButtonChecked(hdlg, DS_SHOWSYSTEMTABLES));

	    sprintf(ci->row_versioning, "%d",
		    IsDlgButtonChecked(hdlg, DS_ROWVERSIONING));
	    ci->disallow_premature =
		IsDlgButtonChecked(hdlg, DS_DISALLOWPREMATURE);
	    ci->lf_conversion =
		IsDlgButtonChecked(hdlg, DS_LFCONVERSION);
	    ci->true_is_minus1 =
		IsDlgButtonChecked(hdlg, DS_TRUEISMINUS1);
	    ci->allow_keyset =
		IsDlgButtonChecked(hdlg, DS_UPDATABLECURSORS);
	    ci->use_server_side_prepare =
		IsDlgButtonChecked(hdlg, DS_SERVERSIDEPREPARE);
	    ci->bytea_as_longvarbinary =
		IsDlgButtonChecked(hdlg, DS_BYTEAASLONGVARBINARY);
	    /*ci->lower_case_identifier = IsDlgButtonChecked(hdlg, DS_LOWERCASEIDENTIFIER); */

	    /* OID Options */
	    sprintf(ci->fake_oid_index, "%d",
		    IsDlgButtonChecked(hdlg, DS_FAKEOIDINDEX));
	    sprintf(ci->show_oid_column, "%d",
		    IsDlgButtonChecked(hdlg, DS_SHOWOIDCOLUMN));

	    /* Datasource Connection Settings */
	    GetDlgItemText(hdlg, DS_CONNSETTINGS, ci->conn_settings,
			   sizeof(ci->conn_settings));
	    if (IDAPPLY == cmd)
	    {
...
static int makeDriversList(HWND lwnd, const ConnInfo * ci)
{
    HMODULE hmodule;
    SQLHENV henv;
    int lcount = 0;
	...
	if (strnicmp(drvname, "postgresql", 10) == 0)
	{
	    ...
	    {
		SendMessage(lwnd, LB_SETCURSEL, (WPARAM) iidx,
			    (LPARAM) 0);
	    }
	    lcount++;
	}
...
LRESULT CALLBACK
manage_dsnProc(HWND hdlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
