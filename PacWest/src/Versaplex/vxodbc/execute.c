 ...
 */
static
RETCODE Exec_with_parameters_resolved(StatementClass * stmt,
				      BOOL * exec_end)
{
    ...
    *exec_end = FALSE;
	...
	("%s: copying statement params: trans_status=%d, len=%d, stmt='%s'\n",
	 func, conn->transact_status, strlen(stmt->statement),
	 stmt->statement);
	...
	*exec_end = TRUE;
	...
	else if (!in_trans)
	{
	    ...
	    {
		SC_set_error(stmt, STMT_EXEC_ERROR,
			     "Handle prepare error", func);
		return SQL_ERROR;
	    }
	}
	/* we are now in a transaction */
	res =
	    CC_send_query(conn, stmt->stmt_with_params, NULL, 0,
			  SC_get_ancestor(stmt));
	...
	*exec_end = TRUE;
    ...
    if (stmt->inaccurate_result || stmt->exec_current_row >= end_row)
    {
	*exec_end = TRUE;
	stmt->exec_current_row = -1;
    } else
	...
	SC_set_error(stmt, STMT_OPTION_VALUE_CHANGED,
		     "cursor updatability changed", func);
...
RETCODE SQL_API PGAPI_Execute(HSTMT hstmt, UWORD flag)
{
	...
	} else
	{
	    ...
	    {
		mylog
		    ("%s: premature statement but return SQL_SUCCESS\n",
		     func);
		retval = SQL_SUCCESS;
		goto cleanup;
	    } else
	    {
		SC_set_error(stmt, STMT_INTERNAL_ERROR,
			     "premature statement so return SQL_ERROR",
			     func);
		retval = SQL_ERROR;
		goto cleanup;
	    }
    ...
    if (!stmt->statement)
    {
	SC_set_error(stmt, STMT_NO_STMTSTRING,
		     "This handle does not have a SQL statement stored in it",
		     func);
...
#define	return	DONT_CALL_RETURN_FROM_HERE???
    ...
    {
	mylog
	    ("%s: recycling statement (should have been done by app)...\n",
	     func);
/******** Is this really NEEDED ? ******/
	...
	if (NOT_YET_PREPARED == stmt->prepared && PROTOCOL_74(ci) && ci->use_server_side_prepare && (ci->bytea_as_longvarbinary ||	/* both lo and bytea are LO */
												     ci->
  ...
  next_param_row:
    ...
    {
	while (apdopts->param_operation_ptr[stmt->exec_current_row] ==
	       SQL_PARAM_IGNORE)
	{
	...
	Int4 num_p =
	    num_params <
	    apdopts->allocated ? num_params : apdopts->allocated;
	...
	for (i = 0; i < num_p; i++)
	{
		...
		if (bind_size > 0)
		    pcVal =
			(SQLLEN *) ((char *) pcVal + offset +
				    bind_size * current_row);
		else
		    pcVal =
			(SQLLEN *) ((char *) pcVal + offset +
				    sizeof(SDWORD) * current_row);
		if (*pcVal == SQL_DATA_AT_EXEC
		    || *pcVal <= SQL_LEN_DATA_AT_EXEC_OFFSET)
...
#define	return	DONT_CALL_RETURN_FROM_HERE???
	...
	SC_set_error(stmt, STMT_SEQUENCE_ERROR,
		     "No execution-time parameters for this statement",
		     func);
	retval = SQL_ERROR;
	goto cleanup;
    }
    ...
    {
	SC_set_error(stmt, STMT_SEQUENCE_ERROR,
		     "Too many execution-time parameters were present",
		     func);
	retval = SQL_ERROR;
	goto cleanup;
    }
	...
	if (retval =
	    PGAPI_Execute(estmt, flag), SQL_NEED_DATA != retval)
	{
	    goto cleanup;
	}
    }
     ...
     */
    i = estmt->current_exec_param >=
	0 ? estmt->current_exec_param + 1 : 0;
	...
	if (apdopts->parameters[i].data_at_exec)
	{
		...
		if (stmt->execute_delegate)
		{
			...
			param_offset_ptr : 0;
		    SQLLEN perrow =
			apdopts->param_bind_type >
			0 ? apdopts->param_bind_type : apdopts->
			parameters[i].buflen;

		    inolog(" offset=%d perrow=%d", offset, perrow);
		    *prgbValue =
			apdopts->parameters[i].buffer + offset +
			estmt->exec_current_row * perrow;
		} else
