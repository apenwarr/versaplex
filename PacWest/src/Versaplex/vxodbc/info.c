			      ...
			      UWORD flag, OID reloid, Int2 attnum)
{
	...
	field_name[MAX_INFO_STRING], field_type_name[MAX_INFO_STRING];
    Int2 field_number, sqltype, concise_type,
	result_cols, decimal_digits;
    Int4 field_type,
	the_type, field_length, mod_length, column_size, ordinal;
...
#define	return	DONT_CALL_RETURN_FROM_HERE???
	...
	} else
	{
	    like_or_eq = eqop;
	    escTableName =
		simpleCatalogEscape(szTableName, cbTableName, NULL,
				    conn);
	    escColumnName =
		simpleCatalogEscape(szColumnName, cbColumnName, NULL,
				    conn);
	}
    }
  retry_public_schema:
	...
	if (0 == attnum
	    && (NULL == escColumnName || like_or_eq != eqop))
	...
	if (escTableName)
	    snprintf_add(columns_query, sizeof(columns_query),
			 " and c.relname %s '%s'", like_or_eq,
			 escTableName);
	my_strcat1(columns_query, " and u.usename %s '%.*s'",
		   like_or_eq, escSchemaName, SQL_NTS);
	if (escColumnName)
	    snprintf_add(columns_query, sizeof(columns_query),
			 " and a.attname %s '%s'", like_or_eq,
    ...
    {
	SC_set_error(stmt, STMT_NO_MEMORY_ERROR,
		     "Couldn't allocate statement for PGAPI_Columns result.",
		     func);
	result = SQL_ERROR;
	goto cleanup;
    }
    ...
    {
	SC_set_error(stmt, STMT_NO_MEMORY_ERROR,
		     "Couldn't allocate memory for PGAPI_Columns result.",
		     func);
	goto cleanup;
    }
    SC_set_Result(stmt, res);
    ...
    /* set the field names */
    QR_set_num_fields(res, result_cols);
    QR_set_field_info_v(res, COLUMNS_CATALOG_NAME, "TABLE_QUALIFIER",
			PG_TYPE_VARCHAR, MAX_INFO_STRING);
    QR_set_field_info_v(res, COLUMNS_SCHEMA_NAME, "TABLE_OWNER",
			PG_TYPE_VARCHAR, MAX_INFO_STRING);
    QR_set_field_info_v(res, COLUMNS_TABLE_NAME, "TABLE_NAME",
			PG_TYPE_VARCHAR, MAX_INFO_STRING);
    QR_set_field_info_v(res, COLUMNS_COLUMN_NAME, "COLUMN_NAME",
			PG_TYPE_VARCHAR, MAX_INFO_STRING);
    QR_set_field_info_v(res, COLUMNS_DATA_TYPE, "DATA_TYPE",
			PG_TYPE_INT2, 2);
    ...
    QR_set_field_info_v(res, COLUMNS_NULLABLE, "NULLABLE", PG_TYPE_INT2,
			2);
    QR_set_field_info_v(res, COLUMNS_REMARKS, "REMARKS",
			PG_TYPE_VARCHAR, INFO_VARCHAR_SIZE);

    QR_set_field_info_v(res, COLUMNS_COLUMN_DEF, "COLUMN_DEF",
			PG_TYPE_VARCHAR, INFO_VARCHAR_SIZE);
    QR_set_field_info_v(res, COLUMNS_SQL_DATA_TYPE, "SQL_DATA_TYPE",
			PG_TYPE_INT2, 2);
    QR_set_field_info_v(res, COLUMNS_SQL_DATETIME_SUB,
			"SQL_DATETIME_SUB", PG_TYPE_INT2, 2);
    QR_set_field_info_v(res, COLUMNS_CHAR_OCTET_LENGTH,
			"CHAR_OCTET_LENGTH", PG_TYPE_INT4, 4);
    QR_set_field_info_v(res, COLUMNS_ORDINAL_POSITION,
			"ORDINAL_POSITION", PG_TYPE_INT4, 4);
    QR_set_field_info_v(res, COLUMNS_IS_NULLABLE, "IS_NULLABLE",
			PG_TYPE_VARCHAR, INFO_VARCHAR_SIZE);
    /* User defined fields */
    QR_set_field_info_v(res, COLUMNS_DISPLAY_SIZE, "DISPLAY_SIZE",
			PG_TYPE_INT4, 4);
    QR_set_field_info_v(res, COLUMNS_FIELD_TYPE, "FIELD_TYPE",
			PG_TYPE_INT4, 4);
    QR_set_field_info_v(res, COLUMNS_AUTO_INCREMENT, "AUTO_INCREMENT",
			PG_TYPE_INT4, 4);
    QR_set_field_info_v(res, COLUMNS_PHYSICAL_NUMBER, "PHYSICAL NUMBER",
			PG_TYPE_INT2, 2);
    QR_set_field_info_v(res, COLUMNS_TABLE_OID, "TABLE OID",
			PG_TYPE_INT4, 4);
	...
	if (!relisaview &&
	    (atoi(ci->show_oid_column) ||
	     strncmp(table_name, POSTGRES_SYS_PREFIX,
		     strlen(POSTGRES_SYS_PREFIX)) == 0))
	{
	    /* For OID fields */
		...
		set_tuplefield_string(&tuple[COLUMNS_SCHEMA_NAME],
				      NULL_STRING);
	    set_tuplefield_string(&tuple[COLUMNS_TABLE_NAME],
				  table_name);
	    set_tuplefield_string(&tuple[COLUMNS_COLUMN_NAME],
				  OID_NAME);
	    ...
	    set_tuplefield_int4(&tuple[COLUMNS_PRECISION],
				pgtype_column_size(stmt, the_type,
						   PG_STATIC,
						   PG_STATIC));
	    set_tuplefield_int4(&tuple[COLUMNS_LENGTH],
				pgtype_buffer_length(stmt, the_type,
						     PG_STATIC,
						     PG_STATIC));
	    set_nullfield_int2(&tuple[COLUMNS_SCALE],
			       pgtype_decimal_digits(stmt, the_type,
						     PG_STATIC));
	    set_nullfield_int2(&tuple[COLUMNS_RADIX],
			       pgtype_radix(stmt, the_type));
	    ...
	    set_tuplefield_int4(&tuple[COLUMNS_ORDINAL_POSITION],
				ordinal);
	    set_tuplefield_string(&tuple[COLUMNS_IS_NULLABLE], "No");
	    set_tuplefield_int4(&tuple[COLUMNS_DISPLAY_SIZE],
				pgtype_display_size(stmt, the_type,
						    PG_STATIC,
						    PG_STATIC));
	    set_tuplefield_int4(&tuple[COLUMNS_FIELD_TYPE], the_type);
	    set_tuplefield_int4(&tuple[COLUMNS_AUTO_INCREMENT], TRUE);
	    set_tuplefield_int2(&tuple[COLUMNS_PHYSICAL_NUMBER],
				OID_ATTNUM);
	...
	PGAPI_GetData(hcol_stmt, 13, internal_asis_type, NULL, 0,
		      &len_needed);
	if (len_needed > 0)
	{
	    mylog("len_needed=%d\n", len_needed);
	    attdef = malloc(len_needed + 1);
	    PGAPI_GetData(hcol_stmt, 13, internal_asis_type, attdef,
			  len_needed + 1, &len_needed);
	...
	case PG_TYPE_INT8:
		...
		if (CC_fake_mss(conn))
		{
		    ...
		    snprintf(tmp, sizeof(tmp), "%s identity",
			     field_type_name);
		    set_tuplefield_string(&tuple[COLUMNS_TYPE_NAME],
					  tmp);
		    break;
		}
	    }
	...
	if (field_type == PG_TYPE_NUMERIC)
	{
		...
		mylog
		    ("%s: field type is NUMERIC: field_type = %d, mod_length=%d, precision=%d, scale=%d\n",
		     func, field_type, mod_length, column_size,
		     decimal_digits);
	    ...
	    (field_type == PG_TYPE_BPCHAR))
	{
	    ...
	    mylog
		("%s: field type is VARCHAR,BPCHAR: field_type = %d, mod_length = %d\n",
	    ...
	    set_tuplefield_int4(&tuple[COLUMNS_CHAR_OCTET_LENGTH],
	...
	if (useStaticPrecision)
	{
		...
		("%s: field type is OTHER: field_type = %d, pgtype_length = %d\n",
		 func, field_type, pgtype_buffer_length(stmt,
							field_type,
							PG_STATIC,
							PG_STATIC));

	    set_tuplefield_int4(&tuple[COLUMNS_PRECISION],
				pgtype_column_size(stmt, field_type,
						   PG_STATIC,
						   PG_STATIC));
	    set_tuplefield_int4(&tuple[COLUMNS_LENGTH],
				pgtype_buffer_length(stmt, field_type,
						     PG_STATIC,
						     PG_STATIC));
	    set_tuplefield_null(&tuple[COLUMNS_CHAR_OCTET_LENGTH]);
	    set_tuplefield_int4(&tuple[COLUMNS_DISPLAY_SIZE],
				pgtype_display_size(stmt, field_type,
						    PG_STATIC,
						    PG_STATIC));
	}
	if (useStaticScale)
	{
	    set_nullfield_int2(&tuple[COLUMNS_SCALE],
			       pgtype_decimal_digits(stmt, field_type,
						     PG_STATIC));
	}
	...
	set_nullfield_int2(&tuple[COLUMNS_RADIX],
			   pgtype_radix(stmt, field_type));
	set_tuplefield_int2(&tuple[COLUMNS_NULLABLE],
			    (Int2) (not_null[0] ==
				    '1' ? SQL_NO_NULLS :
				    pgtype_nullable(stmt, field_type)));
	set_tuplefield_string(&tuple[COLUMNS_REMARKS], NULL_STRING);
	if (attdef && strlen(attdef) > INFO_VARCHAR_SIZE)
	    set_tuplefield_string(&tuple[COLUMNS_COLUMN_DEF],
				  "TRUNCATE");
	else
	    set_tuplefield_string(&tuple[COLUMNS_COLUMN_DEF], attdef);
	set_tuplefield_int2(&tuple[COLUMNS_SQL_DATA_TYPE],
			    concise_type);
	set_nullfield_int2(&tuple[COLUMNS_SQL_DATETIME_SUB],
			   pgtype_to_datetime_sub(stmt, field_type));
	...
	set_tuplefield_int4(&tuple[COLUMNS_AUTO_INCREMENT],
			    auto_unique);
	set_tuplefield_int2(&tuple[COLUMNS_PHYSICAL_NUMBER],
			    field_number);
	set_tuplefield_int4(&tuple[COLUMNS_TABLE_OID], greloid);
	...
	set_tuplefield_int2(&tuple[COLUMNS_DATA_TYPE], sqltype);
	set_tuplefield_string(&tuple[COLUMNS_TYPE_NAME],
			      pgtype_to_name(stmt, the_type, FALSE));
	set_tuplefield_int4(&tuple[COLUMNS_PRECISION],
			    pgtype_column_size(stmt, the_type,
					       PG_STATIC, PG_STATIC));
	set_tuplefield_int4(&tuple[COLUMNS_LENGTH],
...
#define	return	DONT_CALL_RETURN_FROM_HERE???
  ...
  retry_public_schema:
	...
	schema_strcat(columns_query, " and u.nspname = '%.*s'",
		      escSchemaName, SQL_NTS, szTableName, cbTableName,
		      conn);
    else
...
#define	return	DONT_CALL_RETURN_FROM_HERE???
	...
	schema_strcat(table_schemaname, "%.*s", szSchemaName,
		      cbSchemaName, szTableName, cbTableName, conn);
...
#define	return	DONT_CALL_RETURN_FROM_HERE???
	...
	if (conn->schema_support)
	{
	    ...
	    schema_strcat(schema_needed, "%.*s", szFkTableOwner,
			  cbFkTableOwner, szFkTableName, cbFkTableName,
			  conn);
	    escSchemaName =
	...
	while (result == SQL_SUCCESS)
	{
	    ...
	    got_pkname = FALSE;
	    keyresult =
		PGAPI_PrimaryKeys(hpkey_stmt, NULL, 0, schema_fetched,
				  SQL_NTS, pk_table_fetched, SQL_NTS);
	    if (keyresult != SQL_SUCCESS)
	    {
		SC_set_error(stmt, STMT_NO_MEMORY_ERROR,
	...
	if (conn->schema_support)
	{
	    ...
	    schema_strcat(schema_needed, "%.*s", szPkTableOwner,
			  cbPkTableOwner, szPkTableName, cbPkTableName,
			  conn);
	    escSchemaName =
		simpleCatalogEscape(schema_needed, SQL_NTS, NULL, conn);
	    snprintf(tables_query, sizeof(tables_query),
	...
	keyresult =
	    PGAPI_PrimaryKeys(hpkey_stmt, NULL, 0, schema_needed,
			      SQL_NTS, pk_table_needed, SQL_NTS);
	if (keyresult != SQL_SUCCESS)
	{
	    SC_set_error(stmt, STMT_NO_MEMORY_ERROR,
			 "Couldn't get primary keys for PGAPI_ForeignKeys result.",
				 ...
				 SQLSMALLINT cbProcName, UWORD flag)
{
	...
	schema_strcat1(proc_query, " and nspname %s '%.*s'", like_or_eq,
		       escSchemaName, SQL_NTS, szProcName, cbProcName,
		       conn);
