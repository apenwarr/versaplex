			   ...
			   int portno)
{
	...
	snprintf(errmsg, buflen, "%s failed for [%s:%d] ", cmd, host,
		 portno);
    return ret;
}
		...
		long timeout)
{
    ...
    if (self->socket != (SOCKETFD) - 1)
    {
	SOCK_set_error(self, SOCKET_ALREADY_CONNECTED,
	...
	freeaddrinfo_ptr =
	    (freeaddrinfo_func) GetProcAddress(ws2_hnd, "freeaddrinfo");
    if (getaddrinfo_ptr == NULL)
	getaddrinfo_ptr =
	    (getaddrinfo_func) GetProcAddress(ws2_hnd, "getaddrinfo");
    if (getnameinfo_ptr == NULL)
	getnameinfo_ptr =
	    (getnameinfo_func) GetProcAddress(ws2_hnd, "getnameinfo");
#endif
    /*
	...
	if (ret || !addrs)
	{
	    SOCK_set_error(self, SOCKET_HOST_NOT_FOUND,
	...
	struct sockaddr_un *un =
	    (struct sockaddr_un *) &(self->sadr_area);
  ...
  retry:
    ...
    if (self->socket == (SOCKETFD) - 1)
    {
	SOCK_set_error(self, SOCKET_COULD_NOT_CREATE_SOCKET,
	...
	len = sizeof(i);
	if (setsockopt
	    (self->socket, IPPROTO_TCP, TCP_NODELAY, (char *) &i,
	     len) < 0)
	{
    ...
    if (connect
	(self->socket, (struct sockaddr *) &(self->sadr_area),
	 self->sadr_len) < 0)
    {
	int ret, optval;
	...
	do
	{
	    ...
	    ret =
		select((int) self->socket + 1, NULL, &fds, &except_fds,
		       timeout > 0 ? &tm : NULL);
	    gerrno = SOCK_ERRNO;
	    if (0 < ret)
		break;
	...
	{
	    SOCK_set_error(self, SOCKET_COULD_NOT_CONNECT,
			   "Could not connect .. select error occured.");
	...
	{
	    SOCK_set_error(self, SOCKET_COULD_NOT_CONNECT,
			   "Could not connect .. getsockopt error.");
	} else if (optval != 0)
	{
	    ...
	    format_sockerr(errmsg, sizeof(errmsg), optval, "connect",
			   host, port);
	    mylog(errmsg);
...
static int SOCK_wait_for_ready(SocketClass * sock, BOOL output,
			       int retry_count)
{
    int ret, gerrno;
    fd_set fds, except_fds;
    struct timeval tm;
    BOOL no_timeout = (0 != retry_count
		       && (retry_count < 0 || (!sock->ssl)));
	...
	ret =
	    select((int) sock->socket + 1, output ? NULL : &fds,
		   output ? &fds : NULL, &except_fds,
		   no_timeout ? NULL : &tm);
	gerrno = SOCK_ERRNO;
    }
	...
	ret = -1;
	SOCK_set_error(sock,
		       output ? SOCKET_WRITE_TIMEOUT :
		       SOCKET_READ_TIMEOUT,
		       "SOCK_wait_for_ready timeout");
    }
    return ret;
}
...
void SOCK_get_n_char(SocketClass * self, char *buffer, Int4 len)
{
    ...
    if (!buffer)
    {
	SOCK_set_error(self, SOCKET_NULLPOINTER_PARAMETER,
		       "get_n_char was called with NULL-Pointer");
	return;
    }
...
void SOCK_put_n_char(SocketClass * self, const char *buffer, Int4 len)
{
    ...
    {
	SOCK_set_error(self, SOCKET_NULLPOINTER_PARAMETER,
		       "put_n_char was called with NULL-Pointer");
	return;
    }
...
int SOCK_get_int(SocketClass * self, short len)
{
    ...
    default:
	SOCK_set_error(self, SOCKET_GET_INT_WRONG_LENGTH,
...
void SOCK_put_int(SocketClass * self, int value, short len)
{
    ...
    default:
	SOCK_set_error(self, SOCKET_PUT_INT_WRONG_LENGTH,
		       "Cannot write ints of that length");
	return;
    }
}
...
Int4 SOCK_flush_output(SocketClass * self)
{
    ...
    {
	written =
	    send(self->socket, (char *) self->buffer_out + pos,
	...
	if (written < 0)
	{
	    ...
	    SOCK_set_error(self, SOCKET_WRITE_ERROR,
...
UCHAR SOCK_get_next_byte(SocketClass * self)
{
	...
	self->buffer_read_in = 0;
      retry:
	self->buffer_filled_in =
	    recv(self->socket, (char *) self->buffer_in,
		 self->buffer_size, 0);
	gerrno = SOCK_ERRNO;
...
void SOCK_put_next_byte(SocketClass * self, UCHAR next_byte)
{
	...
	do
	{
		...
		case EWOULDBLOCK:
		    retry_count++;
		    if (SOCK_wait_for_ready(self, TRUE, retry_count) >=
			0)
			continue;
		}
		if (0 == self->errornumber)
