...
OID odbc_lo_creat(ConnectionClass * conn, int mode)
{
    ...
    if (!CC_send_function
	(conn, LO_CREAT, &retval, &result_len, 1, argv, 1))
...
int odbc_lo_close(ConnectionClass * conn, int fd)
{
    ...
    if (!CC_send_function
	(conn, LO_CLOSE, &retval, &result_len, 1, argv, 1))
	return -1;
    else
	return retval;
}
...
Int4 odbc_lo_read(ConnectionClass * conn, int fd, char *buf, Int4 len)
{
    ...
    if (!CC_send_function
	(conn, LO_READ, (int *) buf, &result_len, 0, argv, 2))
	return -1;
    else
	return result_len;
}
...
Int4 odbc_lo_write(ConnectionClass * conn, int fd, char *buf, Int4 len)
{
    ...
    if (!CC_send_function
	(conn, LO_WRITE, &retval, &result_len, 1, argv, 2))
	return -1;
    else
	return retval;
}
...
odbc_lo_lseek(ConnectionClass * conn, int fd, int offset, Int4 whence)
{
    ...
    if (!CC_send_function
	(conn, LO_LSEEK, &retval, &result_len, 1, argv, 3))
	return -1;
    else
	return retval;
}
...
Int4 odbc_lo_tell(ConnectionClass * conn, int fd)
{
    ...
    if (!CC_send_function
	(conn, LO_TELL, &retval, &result_len, 1, argv, 1))
	return -1;
    else
	return retval;
}
...
Int4 odbc_lo_unlink(ConnectionClass * conn, OID lobjId)
{
    LO_ARG argv[1];
    Int4 retval, result_len;
    ...
    if (!CC_send_function
	(conn, LO_UNLINK, &retval, &result_len, 1, argv, 1))
	return -1;
    else
	return retval;
}
