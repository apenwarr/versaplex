		    ...
		    SQLLEN cbValueMax, SQLLEN FAR * pcbValue)
{
	...
	ipdopts->parameters[ipar].PGType =
	    sqltype_to_pgtype(stmt, fSqlType);
    switch (fCType)
    {
    case SQL_C_NUMERIC:
	if (cbColDef > 0)
    ...
    apdopts->parameters[ipar].precision =
	ipdopts->parameters[ipar].precision;
    ...
    mylog
	("%s: ipar=%d, paramType=%d, fCType=%d, fSqlType=%d, cbColDef=%d, ibScale=%d,",
	 func, ipar, fParamType, fCType, fSqlType, cbColDef, ibScale);
    ...
    mylog("rgbValue=%p, pcbValue = %p, data_at_exec = %d\n", rgbValue,
	  pcbValue, apdopts->parameters[ipar].data_at_exec);
		    ...
		    SQLSMALLINT FAR * pfNullable)
{
    ...
    {
	inolog("num_params=%d\n", stmt->num_params);
	SC_set_error(stmt, STMT_BAD_PARAMETER_NUMBER_ERROR,
...
#define	return	DONT_CALL_RETURN_FROM_HERE???
	...
	inolog("[%d].SQLType=%d .PGType=%d\n", ipar,
	       ipdopts->parameters[ipar].SQLType,
	       ipdopts->parameters[ipar].PGType);
	...
	else
	{
	    ...
	    SC_set_error(stmt, STMT_EXEC_ERROR,
			 "Unfortunatley couldn't get this paramater's info",
			 func);
	    goto cleanup;
	}
    }
	...
	*pibScale = 0;
	if (ipdopts->parameters[ipar].SQLType)
	    ...
	    *pibScale =
		pgtype_scale(stmt, ipdopts->parameters[ipar].PGType,
			     -1);
    }

    if (pfNullable)
	*pfNullable =
	    pgtype_nullable(stmt, ipdopts->parameters[ipar].paramType);
  cleanup:
#undef	return
    if (stmt->internal)
	ret = DiscardStatementSvp(stmt, ret, FALSE);
...
RETCODE SQL_API PGAPI_NumParams(HSTMT hstmt, SQLSMALLINT FAR * pcpar)
{
	...
	*pcpar = 0;
    ...
    {
	SC_set_error(stmt, STMT_EXEC_ERROR,
		     "parameter count address is null", func);
	return SQL_ERROR;
    }
	...
	*pcpar = stmt->num_params;
	...
	SC_set_error(stmt, STMT_SEQUENCE_ERROR,
		     "PGAPI_NumParams called with no statement ready.",
		     func);
	return SQL_ERROR;
    } else
    {
	...
	stmt->proc_return = 0;
	SC_scanQueryAndCountParams(stmt->statement, SC_get_conn(stmt),
				   NULL, pcpar, &multi, &proc_return);
...
void extend_parameter_bindings(APDFields * self, int num_params)
{
    CSTR func = "extend_parameter_bindings";
    ParameterInfoClass *new_bindings;

    mylog
	("%s: entering ... self=%p, parameters_allocated=%d, num_params=%d,%p\n",
    ...
    {
	new_bindings =
	    (ParameterInfoClass *) realloc(self->parameters,
	...
	memset(&new_bindings[self->allocated], 0,
	       sizeof(ParameterInfoClass) * (num_params -
					     self->allocated));
...
void extend_iparameter_bindings(IPDFields * self, int num_params)
{
    ...
    {
	new_bindings =
	    (ParameterImplClass *) realloc(self->parameters,
	...
	memset(&new_bindings[self->allocated], 0,
	       sizeof(ParameterImplClass) * (num_params -
					     self->allocated));
...
extend_putdata_info(PutDataInfo * self, int num_params, BOOL shrink)
{
	...
	new_pdata =
	    (PutDataClass *) realloc(self->pdata,
