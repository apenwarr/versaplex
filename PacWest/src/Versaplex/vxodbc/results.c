...
static BOOL SC_pre_execute_ok(StatementClass * stmt, BOOL build_fi,
			      int col_idx, const char *func)
{
    ...
    mylog("%s: result = %p, status = %d, numcols = %d\n", func, result,
	  stmt->status, result != NULL ? QR_NumResultCols(result) : -1);
 ...
 */
RETCODE SQL_API
PGAPI_NumResultCols(HSTMT hstmt, SQLSMALLINT FAR * pccol)
{
...
#define	return	DONT_CALL_RETURN_FROM_HERE???
    ...
    {
	SQLSMALLINT fType =
	    stmt->options.use_bookmarks ==
	    SQL_UB_VARIABLE ? SQL_BINARY : SQL_INTEGER;
    ...
    if (!FI_is_applicable(fi) && !stmt->catalog_result
	...
	mylog
	    ("PARSE: DescribeCol: icol=%d, stmt=%p, stmt->nfld=%d, stmt->fi=%p\n",
	 ...
	 * old fashioned way.
	 */
	BOOL build_fi = PROTOCOL_74(ci) && (NULL != pfNullable
					    || NULL != pfSqlType);
	fi = NULL;
	...
	{
	    SC_set_error(stmt, STMT_INVALID_COLUMN_NUMBER_ERROR,
			 "Invalid column number in DescribeCol.", NULL);
	    snprintf(buf, sizeof(buf), "Col#=%d, #Cols=%d,%d keys=%d",
		     icol, QR_NumResultCols(res),
		     QR_NumPublicResultCols(res), res->num_key_fields);
	...
	*pcbColName = len;
	...
	if (len >= cbColNameMax)
	{
	    ...
	    SC_set_error(stmt, STMT_TRUNCATED,
			 "The buffer was too small for the colName.",
			 func);
	}
    }
	...
	*pibScale = (SQLSMALLINT) decimal_digits;
	...
	if (SC_has_outer_join(stmt))
	    ...
	    *pfNullable =
		fi ? fi->nullable : pgtype_nullable(stmt, fieldtype);
		    ...
		    SQLSMALLINT FAR * pcbDesc, SQLLEN FAR * pfDesc)
{
	...
	*pcbDesc = 0;
	...
	if (SC_parsed_status(stmt) != STMT_PARSE_FATAL && irdflds->fi)
	{
	    ...
	    {
		SC_set_error(stmt, STMT_INVALID_COLUMN_NUMBER_ERROR,
			     "Invalid column number in ColAttributes.",
			     func);
		return SQL_ERROR;
	    }
	}
	...
	value = (fi
	    ...
	    display_size : pgtype_display_size(stmt, field_type,
	...
	inolog("fi=%p", fi);
	if (fi)
	    inolog(" (%s,%s)", PRINT_NAME(fi->column_alias),
		   PRINT_NAME(fi->column_name));
	p = fi ? (NAME_IS_NULL(fi->column_alias) ?
		  SAFE_NAME(fi->column_name) : GET_NAME(fi->
							column_alias)) :
	...
	value = (fi
		 && fi->length >
		 0) ? fi->length : pgtype_buffer_length(stmt,
							field_type,
							col_idx,
							unknown_sizes);
	if (0 > value)
	...
	if (SC_has_outer_join(stmt))
	    ...
	    value =
		fi ? fi->nullable : pgtype_nullable(stmt, field_type);
	...
	value = (fi
		 && fi->column_size >
		 0) ? fi->column_size : pgtype_column_size(stmt,
							   field_type,
							   col_idx,
							   unknown_sizes);
	if (value < 0)
	...
	value =
		  ...
		  updatable ? SQL_ATTR_WRITE : SQL_ATTR_READONLY)
	    : (QR_get_attid(res, col_idx) >
	       0 ? SQL_ATTR_WRITE : (PROTOCOL_74(ci) ? SQL_ATTR_READONLY
				     : SQL_ATTR_READWRITE_UNKNOWN));
	if (SQL_ATTR_READONLY != value)
	{
	    const char *name =
		fi ? SAFE_NAME(fi->column_name) : QR_get_fieldname(res,
								   col_idx);
	...
	p = fi ? SAFE_NAME(fi->column_name) : QR_get_fieldname(res,
							       col_idx);
	...
	value = (fi
		 && fi->length >
		 0) ? fi->length : pgtype_desclength(stmt, field_type,
						     col_idx,
						     unknown_sizes);
	if (-1 == value)
	    value = 0;
	...
	value = (fi
		 && fi->length >
		 0) ? fi->length : pgtype_transfer_octet_length(stmt,
								field_type,
								col_idx,
								unknown_sizes);
	if (-1 == value)
	...
	if (value = FI_precision(fi), value <= 0)
	    value =
		pgtype_precision(stmt, field_type, col_idx,
				 unknown_sizes);
	if (value < 0)
	    value = 0;
	      ...
	      PTR rgbValue, SQLLEN cbValueMax, SQLLEN FAR * pcbValue)
{
	...
	SC_set_error(stmt, STMT_STATUS_ERROR,
		     "GetData can only be called after the successful execution on a SQL statement",
		     func);
	return SQL_ERROR;
    }
	...
	{
	    SC_set_error(stmt, STMT_STATUS_ERROR,
			 "GetData can't determine the type via ARD",
			 func);
	    return SQL_ERROR;
	}
    } else
...
#define	return	DONT_CALL_RETURN_FROM_HERE???
    ...
    mylog
	("**** %s: icol = %d, target_type = %d, field_type = %d, value = '%s'\n",
    ...
    result = copy_and_convert_field(stmt, field_type, value,
				    target_type, rgbValue, cbValueMax,
				    pcbValue, pcbValue);
 ...
 *		advances the cursor.
 */
RETCODE SQL_API PGAPI_Fetch(HSTMT hstmt)
{
    ...
    {
	SC_set_error(stmt, STMT_INVALID_CURSOR_STATE_ERROR,
		     "Null statement result in PGAPI_Fetch.", func);
	return SQL_ERROR;
    }
    ...
    {
	SC_set_error(stmt, STMT_COLNUM_ERROR,
		     "Not allowed to bind a bookmark column when using PGAPI_Fetch",
		     func);
	return SQL_ERROR;
    }
    ...
    {
	SC_set_error(stmt, STMT_SEQUENCE_ERROR,
		     "Can't fetch while statement is still executing.",
		     func);
	return SQL_ERROR;
    }
    ...
    {
	SC_set_error(stmt, STMT_SEQUENCE_ERROR,
		     "Fetch can only be called after the successful execution on a SQL statement",
		     func);
	return SQL_ERROR;
    }
...
SC_pos_reload_needed(StatementClass * stmt, SQLULEN req_size,
		     UDWORD flag);
SQLLEN getNthValid(const QResultClass * res, SQLLEN sta,
		   UWORD orientation, SQLULEN nth, SQLLEN * nearest)
{
    ...
    inolog("get %dth Valid data from %d to %s [dlt=%d]", nth, sta,
	   orientation == SQL_FETCH_PRIOR ? "backward" : "forward",
	   res->dl_count);
    if (0 == res->dl_count)
    {
	...
	*nearest = sta - 1 + nth;
	if (SQL_FETCH_PRIOR == orientation)
	{
	    for (i = res->dl_count - 1;
		 i >= 0 && *nearest <= (SQLLEN) deleted[i]; i--)
	    {
		inolog("deleted[%d]=%d\n", i, deleted[i]);
	...
	} else
	{
		...
		num_tuples = INT_MAX;
	    for (i = 0;
		 i < res->dl_count && *nearest >= (SQLLEN) deleted[i];
		 i++)
	    {
		if (sta <= (SQLLEN) deleted[i])
		    (*nearest)++;
	...
	for (i = sta, keyset = res->keyset + sta; i >= 0; i--, keyset--)
	{
	    if (0 ==
		(keyset->
		 status & (CURS_SELF_DELETING | CURS_SELF_DELETED |
			   CURS_OTHER_DELETED)))
	    {
		*nearest = i;
		inolog(" nearest=%d\n", *nearest);
...
static void
move_cursor_position_if_needed(StatementClass * self,
			       QResultClass * res)
{
    SQLLEN move_offset;
	...
	QR_get_rowstart_in_cache(res) <= res->num_cached_rows)
    {
	QR_set_next_in_cache(res,
			     (QR_get_rowstart_in_cache(res) <
			      0) ? 0 : QR_get_rowstart_in_cache(res));
	return;
    }
    if (0 == move_offset)
	return;
		    ...
		    SQLLEN bookmark_offset, SQLLEN rowsetSize)
{
    ...
    SQLLEN save_rowset_start, rowset_start;
    RETCODE result = SQL_SUCCESS;
	...
	if (fFetchType != SQL_FETCH_NEXT)
	{
	    SC_set_error(stmt, STMT_FETCH_OUT_OF_RANGE,
			 "The fetch type for PGAPI_ExtendedFetch isn't allowed with ForwardOnly cursor.",
			 func);
	    return SQL_ERROR;
	}
    }
    ...
    {
	SC_set_error(stmt, STMT_STATUS_ERROR,
		     "ExtendedFetch can only be called after the successful execution on a SQL statement",
		     func);
	return SQL_ERROR;
    }
	...
	*pcrow = 0;
	...
	else if (res->keyset)
	{
		...
		if (getNthValid(res, SC_get_rowset_start(stmt),
				...
				&rowset_start) <= 0)
		{
		EXTFETCH_RETURN_EOF(stmt, res)} else
	...
	if (SC_get_rowset_start(stmt) >= num_tuples)
	{
	    ...
	    {
		SC_set_error(stmt, STMT_POS_BEFORE_RECORDSET,
			     "fetch prior from eof and before the beginning",
			     func);
	    }
	    SC_set_rowset_start(stmt,
				num_tuples <=
				0 ? 0 : (num_tuples - rowsetSize),
				TRUE);
	} else if (QR_haskeyset(res))
	{
	    if (i =
		getNthValid(res, SC_get_rowset_start(stmt) - 1,
			    SQL_FETCH_PRIOR, rowsetSize, &rowset_start),
		i < -1)
	    {
		SC_set_error(stmt, STMT_POS_BEFORE_RECORDSET,
			     "fetch prior and before the beggining",
			     func);
		SC_set_rowset_start(stmt, 0, TRUE);
	    } else if (i <= 0)
	    {
	    EXTFETCH_RETURN_BOF(stmt, res)} else
		should_set_rowset_start = TRUE;
	} else if (SC_get_rowset_start(stmt) < rowsetSize)
	{
	    SC_set_error(stmt, STMT_POS_BEFORE_RECORDSET,
			 "fetch prior from eof and before the beggining",
			 func);
	    SC_set_rowset_start(stmt, 0, TRUE);
	} else
	...
	SC_set_rowset_start(stmt,
			    num_tuples <=
			    0 ? 0 : (num_tuples - rowsetSize), TRUE);
	break;

    case SQL_FETCH_ABSOLUTE:
	mylog
	    ("SQL_FETCH_ABSOLUTE: num_tuples=%d, currtuple=%d, irow=%d\n",
	...
	{
	    if (getNthValid(res, 0, SQL_FETCH_NEXT, irow, &rowset_start)
		<= 0)
	    {
	    EXTFETCH_RETURN_EOF(stmt, res)} else
		should_set_rowset_start = TRUE;
	...
	{
	    if (getNthValid
		(res, num_tuples - 1, SQL_FETCH_PRIOR, -irow,
		 &rowset_start) <= 0)
	    {
	    EXTFETCH_RETURN_BOF(stmt, res)} else
	    {
	...
	{
	    if (getNthValid
		(res, SC_get_rowset_start(stmt) + 1, SQL_FETCH_NEXT,
		 irow, &rowset_start) <= 0)
	    {
	    EXTFETCH_RETURN_EOF(stmt, res)} else
		should_set_rowset_start = TRUE;
	...
	{
	    if (getNthValid
		(res, SC_get_rowset_start(stmt) - 1, SQL_FETCH_PRIOR,
		 -irow, &rowset_start) <= 0)
	    {
	    EXTFETCH_RETURN_BOF(stmt, res)} else
		should_set_rowset_start = TRUE;
    ...
    case SQL_FETCH_BOOKMARK:
	{
	    ...
	    {
		if (getNthValid
		    (res, bidx, SQL_FETCH_NEXT, bookmark_offset + 1,
		     &rowset_start) <= 0)
		{
		EXTFETCH_RETURN_EOF(stmt, res)} else
		    should_set_rowset_start = TRUE;
		...
		if (getNthValid
		    (res, bidx, SQL_FETCH_PRIOR, 1 - bookmark_offset,
		     &rowset_start) <= 0)
	    {
		stmt->currTuple = -1;
	    EXTFETCH_RETURN_BOF(stmt, res)} else
    ...
    default:
	SC_set_error(stmt, STMT_FETCH_OUT_OF_RANGE,
		     "Unsupported PGAPI_ExtendedFetch Direction", func);
	return SQL_ERROR;
    }
...
#define	return DONT_CALL_RETURN_FROM_HERE???
	...
	getNthValid(res, rowset_start, SQL_FETCH_NEXT, rowsetSize,
		    &rowset_end);
	req_size = rowset_end - rowset_start + 1;
	if (SQL_CURSOR_KEYSET_DRIVEN == stmt->options.cursor_type)
	{
	    if (fFetchType != SQL_FETCH_NEXT ||
		QR_get_rowstart_in_cache(res) + req_size >
		QR_get_num_cached_tuples(res))
		flag = 1;
	}
	if (SQL_RD_ON == stmt->options.retrieve_data || flag != 0)
	{
    ...
    for (i = 0, fc_io = 0;
	 SQL_NO_DATA_FOUND != result && SQL_ERROR != result; currp++)
    {
	fc_io++;
...
RETCODE SQL_API PGAPI_MoreResults(HSTMT hstmt)
{
...
static void getTid(const QResultClass * res, SQLLEN index,
		   UInt4 * blocknum, UInt2 * offset)
{
...
SQLLEN ReplaceCachedRows(TupleField * otuple, const TupleField * ituple,
			 int num_fields, SQLLEN num_rows)
{
    SQLLEN i;
...
static
int MoveCachedRows(TupleField * otuple, TupleField * ituple,
		   Int2 num_fields, SQLLEN num_rows)
{
    int i;
...
static BOOL tupleExists(const StatementClass * stmt,
			const KeySet * keyset)
{
    char selstr[256];
	...
	snprintf(selstr, sizeof(selstr),
		 "select 1 from \"%s\".\"%s\" where ctid = '(%d,%d)'",
		 SAFE_NAME(ti->schema_name), SAFE_NAME(ti->table_name),
		 keyset->blocknum, keyset->offset);
    else
	snprintf(selstr, sizeof(selstr),
		 "select 1 from \"%s\" where ctid = '(%d,%d)'",
		 SAFE_NAME(ti->table_name), keyset->blocknum,
		 keyset->offset);
...
static void AddAdded(StatementClass * stmt, QResultClass * res,
		     SQLLEN index, const TupleField * tuple_added)
{
...
static void RemoveUpdatedAfterTheKey(QResultClass *, SQLLEN,
				     const KeySet *);
...
static void RemoveAdded(QResultClass * res, SQLLEN index)
{
    ...
    if (mv_count > 0)
    {
	memmove(added_keyset, added_keyset + 1,
		mv_count * sizeof(KeySet));
	memmove(added_tuples, added_tuples + num_fields,
		mv_count * num_fields * sizeof(TupleField));
    }
...
int AddDeleted(QResultClass * res, SQLULEN index, KeySet * keyset)
{
	...
	dl_count = 0;
	new_alloc = 10;
	QR_MALLOC_return_with_error(res->deleted, SQLULEN,
				    sizeof(SQLULEN) * new_alloc, res,
				    "Deleted index malloc error",
				    FALSE);
	QR_MALLOC_return_with_error(res->deleted_keyset, KeySet,
				    sizeof(KeySet) * new_alloc, res,
				    "Deleted keyset malloc error",
				    FALSE);
	deleted = res->deleted;
	...
	if (dl_count >= res->dl_alloc)
	{
	    ...
	    res->dl_alloc = 0;
	    QR_REALLOC_return_with_error(res->deleted, SQLULEN,
					 sizeof(SQLULEN) * new_alloc,
					 res,
					 "Dleted index realloc error",
					 FALSE);
	    deleted = res->deleted;
	    QR_REALLOC_return_with_error(res->deleted_keyset, KeySet,
					 sizeof(KeySet) * new_alloc,
					 res,
					 "Dleted KeySet realloc error",
					 FALSE);
	    deleted_keyset = res->deleted_keyset;
	    res->dl_alloc = new_alloc;
	}
	/* sort deleted indexes in ascending order */
	for (i = 0, deleted = res->deleted, deleted_keyset =
	     res->deleted_keyset; i < dl_count;
	     i++, deleted++, deleted_keyset += num_fields)
	{
	    if (index < *deleted)
		break;
	}
	memmove(deleted + 1, deleted, sizeof(SQLLEN) * (dl_count - i));
	memmove(deleted_keyset + 1, deleted_keyset,
		sizeof(KeySet) * (dl_count - i));
    }
    *deleted = index;
...
static void RemoveDeleted(QResultClass * res, SQLLEN index)
{
    ...
    {
	if (pidx == res->deleted[i] || midx == res->deleted[i])
	{
		...
		memmove(deleted, deleted + 1,
			mv_count * sizeof(SQLULEN));
		memmove(deleted_keyset, deleted_keyset + 1,
			mv_count * sizeof(KeySet));
	    }
	    res->dl_count--;
	    rm_count++;
...
static void CommitDeleted(QResultClass * res)
{
    ...
    for (i = 0, deleted = res->deleted, deleted_keyset =
	 res->deleted_keyset; i < res->dl_count;
	 i++, deleted++, deleted_keyset++)
    {
...
static BOOL enlargeUpdated(QResultClass * res, Int4 number,
			   const StatementClass * stmt)
{
    Int2 alloc;
    SQLULEN *updated;
...
static void RemoveUpdatedAfterTheKey(QResultClass * res, SQLLEN index,
				     const KeySet * keyset)
{
    ...
    mylog("RemoveUpdatedAfterTheKey %d,(%d,%d)\n", index,
	  keyset ? keyset->blocknum : 0, keyset ? keyset->offset : 0);
    if (index < 0)
    {
	midx = index;
	pidx = num_read - index - 1;
	...
	if (pidx == *updated || midx == *updated)
	{
	    ...
	    if (mv_count > 0)
	    {
		memmove(updated, updated + 1,
			sizeof(SQLULEN) * mv_count);
		memmove(updated_keyset, updated_keyset + 1,
			sizeof(KeySet) * mv_count);
		if (updated_tuples)
		    memmove(updated_tuples, updated_tuples + num_fields,
			    sizeof(TupleField) * num_fields * mv_count);
	    }
	    res->up_count--;
	    rm_count++;
	}
...
static void DiscardRollback(StatementClass * stmt, QResultClass * res)
{
	...
	if (kres_is_valid)
	{
	    keyset[kres_ridx].status &=
		~(CURS_SELF_DELETING | CURS_SELF_UPDATING |
		  CURS_SELF_ADDING);
	    keyset[kres_ridx].status |=
		((status &
		  (CURS_SELF_DELETING | CURS_SELF_UPDATING |
			 ...
			 BOOL partial)
{
	...
	if (kres_is_valid)
	{
		...
		{
		    TupleField *tuple =
			res->backend_tuples + res->num_fields * ridx;
				     ...
				     const char *tidval)
{
	...
	*oideqstr = '\0';
	...
	if (latest)
	{
	    if (NAME_IS_VALID(ti->schema_name))
		snprintf(selstr, len,
			 "%s where ctid = currtid2('\"%s\".\"%s\"', '%s') %s",
			 stmt->load_statement,
			 SAFE_NAME(ti->schema_name),
	    ...
	    snprintf(selstr, len, "%s where ctid = '%s' %s",
		     stmt->load_statement, tidval, oideqstr);
    } else if ((flag & USE_INSERTED_TID) != 0)
	snprintf(selstr, len, "%s where ctid = currtid(0, '(0,0)') %s",
		 stmt->load_statement, oideqstr);
    ...
    {
	SC_set_error(stmt, STMT_INTERNAL_ERROR,
		     "can't find the add and updating row because of the lack of oid",
		     func);
	goto cleanup;
    }
...
static RETCODE
SC_pos_reload_with_tid(StatementClass * stmt, SQLULEN global_ridx,
		       UInt2 * count, Int4 logKind, const char *tid)
{
	...
	*count = 0;
    ...
    {
	SC_set_error(stmt, STMT_INVALID_CURSOR_STATE_ERROR,
		     "Null statement result in SC_pos_reload.", func);
	return SQL_ERROR;
    }
    ...
    {
	data_in_cache = FALSE;
	SC_set_error(stmt, STMT_ROW_OUT_OF_RANGE,
		     "the target rows is out of the rowset", func);
	return SQL_ERROR;
    }
	...
	if (NULL == tid)
	{
	    use_ctid = FALSE;
	    mylog
		("The tuple is currently being added and can't use ctid\n");
	}
    }
	...
	SC_set_error(stmt, STMT_INVALID_OPTION_IDENTIFIER,
		     "the statement is read-only", func);
	return SQL_ERROR;
    }
	...
	{
	    SC_set_error(stmt, STMT_ROW_VERSION_CHANGED,
			 "the row was already deleted ?", func);
	...
	qres =
	    positioned_load(stmt, use_ctid ? LATEST_TUPLE_LOAD : 0,
			    &oidint, use_ctid ? tidval : NULL);
    if (!QR_command_maybe_successful(qres))
    {
	ret = SQL_ERROR;
	SC_replace_error_with_res(stmt, STMT_ERROR_TAKEN_FROM_BACKEND,
				  "positioned_load failed", qres, TRUE);
    } else
    {
	...
	if (rcnt == 1)
	{
		...
		KeySetSet(tuple_new, qres->num_fields,
			  res->num_key_fields, res->keyset + kres_ridx);
	    }
	    if (data_in_cache)
		MoveCachedRows(tuple_old, tuple_new, effective_fields,
			       1);
	    ret = SQL_SUCCESS;
	} else
	{
...
RETCODE
SC_pos_reload(StatementClass * stmt, SQLULEN global_ridx, UInt2 * count,
	      Int4 logKind)
{
    return SC_pos_reload_with_tid(stmt, global_ridx, count, logKind,
				  NULL);
}

static const int pre_fetch_count = 32;
static SQLLEN LoadFromKeyset(StatementClass * stmt, QResultClass * res,
			     int rows_per_fetch, SQLLEN limitrow)
{
    ...
    for (i = SC_get_rowset_start(stmt), kres_ridx =
	 GIdx2KResIdx(i, stmt, res), rowc = 0;; i++)
    {
	if (i >= limitrow)
	{
	    if (!rowc)
	...
	if (rowc < 0 || rowc >= keys_per_fetch)
	{
		...
		for (j = 0; j < QR_get_num_total_read(qres); j++)
		{
		    ...
		    for (k = SC_get_rowset_start(stmt); k < limitrow;
			 k++)
		    {
			if (oid == getOid(res, k))
			{
	...
	if (!rowc)
	{
		...
		if (prepare)
		{
			...
			sprintf(sval, ") as %s where ctid in ",
				stmt->load_statement);
			sval = strchr(sval, '\0');
			for (j = 0; j < keys_per_fetch; j++)
			{
		     ...
		     UDWORD flag)
{
    ...
    {
	SC_set_error(stmt, STMT_INVALID_CURSOR_STATE_ERROR,
		     "Null statement result in SC_pos_reload_needed.",
		     func);
	return SQL_ERROR;
    }
    if (SC_update_not_ready(stmt))
    ...
    {
	rows_per_fetch =
	    ((pre_fetch_count - 1) / req_rows_size + 1) * req_rows_size;
	...
	ClearCachedRows(res->backend_tuples, res->num_fields,
			res->num_cached_rows);
	...
	if (SQL_RD_ON != stmt->options.retrieve_data)
	    return SQL_SUCCESS;
	for (i = SC_get_rowset_start(stmt), kres_ridx =
	     GIdx2KResIdx(i, stmt, res); i < limitrow; i++, kres_ridx++)
	{
	    if (0 ==
		(res->keyset[kres_ridx].
		 status & (CURS_SELF_DELETING | CURS_SELF_DELETED |
			   CURS_OTHER_DELETED)))
    ...
    if (rowc =
	LoadFromKeyset(stmt, res, rows_per_fetch, limitrow), rowc < 0)
    {
	return SQL_ERROR;
    }
    for (i = SC_get_rowset_start(stmt), kres_ridx =
	 GIdx2KResIdx(i, stmt, res); i < limitrow; i++)
    {
	if (0 != (res->keyset[kres_ridx].status & CURS_NEEDS_REREAD))
	{
	       ...
	       const char *tidval)
{
    ...
    {
	SC_set_error(stmt, STMT_INVALID_CURSOR_STATE_ERROR,
		     "Null statement result in SC_pos_newload.", func);
	return SQL_ERROR;
    }
    if (SC_update_not_ready(stmt))
	parse_statement(stmt, TRUE);	/* not preferable */
	...
	positioned_load(stmt,
			(tidRef
			 && NULL == tidval) ? USE_INSERTED_TID : 0,
			oidint, tidRef ? tidval : NULL);
    if (!qres || !QR_command_maybe_successful(qres))
    {
	SC_set_error(stmt, STMT_ERROR_TAKEN_FROM_BACKEND,
		     "positioned_load in pos_newload failed", func);
    } else
    {
	...
	if (count == 1)
	{
		...
		if (!QR_get_cursor(res))
		{
		    appendKey = TRUE;
		    if (num_total_rows ==
			CacheIdx2GIdx(num_cached_rows, stmt, res))
			appendData = TRUE;
		    else
		    {
			inolog
			    ("total %d <> backend %d - base %d + start %d cursor_type=%d\n",
		...
		if (num_cached_rows >= res->count_backend_allocated)
		{
		    ...
		    res->backend_tuples =
			(TupleField *) realloc(res->backend_tuples,
					       res->num_fields *
					       sizeof(TupleField) *
					       tuple_size);
		...
		tuple_old =
		    res->backend_tuples +
		    res->num_fields * num_cached_rows;
	    ...
	    SQLSETPOSIROW irow, SQLULEN global_ridx)
{
	...
	if (cmdstr && sscanf(cmdstr, "UPDATE %d", &updcnt) == 1)
	{
	    if (updcnt == 1)
	    {
		...
		ret =
		    SC_pos_reload_with_tid(stmt, global_ridx,
					   (UInt2 *) 0, SQL_UPDATE,
					   tidval);
		if (SQL_ERROR != ret)
	    ...
	    {
		SC_set_error(stmt, STMT_ROW_VERSION_CHANGED,
			     "the content was changed before updation",
			     func);
		ret = SQL_ERROR;
		if (stmt->options.cursor_type ==
	...
	{
	    SC_set_error(stmt, STMT_ERROR_TAKEN_FROM_BACKEND,
			 "SetPos update return error", func);
	}
    }
    return ret;
}
...
static RETCODE pos_update_callback(RETCODE retcode, void *para)
{
	...
	ret =
	    irow_update(ret, s->stmt, s->qstmt, s->irow,
			s->global_ridx);
	inolog("irow_update ret=%d,%d\n", ret,
	       SC_get_errornumber(s->qstmt));
	if (ret != SQL_SUCCESS)
    ...
    {
	SC_set_error(s->stmt, STMT_ROW_OUT_OF_RANGE,
		     "the target rows is out of the rowset", func);
	inolog("gidx=%d num_keys=%d kresidx=%d\n", s->global_ridx,
	       s->res->num_cached_keys, kres_ridx);
	return SQL_ERROR;
    }
	      ...
	      SQLSETPOSIROW irow, SQLULEN global_ridx)
{
    ...
    {
	SC_set_error(s.stmt, STMT_INVALID_CURSOR_STATE_ERROR,
		     "Null statement result in SC_pos_update.", func);
	return SQL_ERROR;
    }
    mylog("POS UPDATE %d+%d fi=%p ti=%p\n", s.irow,
	  QR_get_rowstart_in_cache(s.res), fi, s.stmt->ti);
    if (SC_update_not_ready(stmt))
	parse_statement(s.stmt, TRUE);	/* not preferable */
    if (!s.stmt->updatable)
    {
	s.stmt->options.scroll_concurrency = SQL_CONCUR_READ_ONLY;
	SC_set_error(s.stmt, STMT_INVALID_OPTION_IDENTIFIER,
		     "the statement is read-only", func);
	return SQL_ERROR;
    }
    kres_ridx = GIdx2KResIdx(s.global_ridx, s.stmt, s.res);
	...
	if (used = bindings[i].used, used != NULL)
	{
	    ...
	    {
		if (upd_cols)
		    sprintf(updstr, "%s, \"%s\" = ?", updstr,
			    GET_NAME(fi[i]->column_name));
		else
		    sprintf(updstr, "%s \"%s\" = ?", updstr,
			    GET_NAME(fi[i]->column_name));
		upd_cols++;
	...
	{
	    SC_set_error(s.stmt, STMT_NO_MEMORY_ERROR,
			 "internal AllocStmt error", func);
	    return SQL_ERROR;
	}
	...
	if (ret == SQL_NEED_DATA)
	{
	    ...
	    enqueueNeedDataCallback(s.stmt, pos_update_callback,
				    cbdata);
	    return ret;
	}
	      ...
	      SQLSETPOSIROW irow, SQLULEN global_ridx)
{
    ...
    {
	SC_set_error(stmt, STMT_INVALID_CURSOR_STATE_ERROR,
		     "Null statement result in SC_pos_delete.", func);
	return SQL_ERROR;
    }
    if (SC_update_not_ready(stmt))
	parse_statement(stmt, TRUE);	/* not preferable */
    ...
    if (NAME_IS_VALID(ti->schema_name))
	sprintf(dltstr,
		"delete from \"%s\".\"%s\" where ctid = '(%u, %u)'",
		SAFE_NAME(ti->schema_name), SAFE_NAME(ti->table_name),
		blocknum, offset);
    else
	...
	if (cmdstr && sscanf(cmdstr, "DELETE %d", &dltcnt) == 1)
	{
	    if (dltcnt == 1)
	    {
		RETCODE tret =
		    SC_pos_reload(stmt, global_ridx, (UInt2 *) 0,
				  SQL_DELETE);
	    ...
	    {
		SC_set_error(stmt, STMT_ROW_VERSION_CHANGED,
			     "the content was changed before deletion",
			     func);
		ret = SQL_ERROR;
		if (stmt->options.cursor_type ==
		    SQL_CURSOR_KEYSET_DRIVEN)
    ...
    {
	SC_set_error(stmt, STMT_ERROR_TAKEN_FROM_BACKEND,
		     "SetPos delete return error", func);
    }
    if (qres)
	QR_Destructor(qres);
	    ...
	    SQLLEN addpos)
{
	    ...
	    addcnt == 1)
	{
		...
		snprintf(buf, sizeof(buf), FORMAT_LEN,
			 SC_make_bookmark(addpos));
	...
	{
	    SC_set_error(stmt, STMT_ERROR_TAKEN_FROM_BACKEND,
			 "SetPos insert return error", func);
	}
    }
    return ret;
}
...
static RETCODE pos_add_callback(RETCODE retcode, void *para)
{
    RETCODE ret = retcode;
...
RETCODE SC_pos_add(StatementClass * stmt, SQLSETPOSIROW irow)
{
    ...
    {
	SC_set_error(s.stmt, STMT_INVALID_CURSOR_STATE_ERROR,
		     "Null statement result in SC_pos_add.", func);
	return SQL_ERROR;
    }
    if (SC_update_not_ready(stmt))
	parse_statement(s.stmt, TRUE);	/* not preferable */
	...
	sprintf(addstr, "insert into \"%s\".\"%s\" (",
		SAFE_NAME(s.stmt->ti[0]->schema_name),
		SAFE_NAME(s.stmt->ti[0]->table_name));
    else
	sprintf(addstr, "insert into \"%s\" (",
		SAFE_NAME(s.stmt->ti[0]->table_name));
    if (PGAPI_AllocStmt(conn, &hstmt) != SQL_SUCCESS)
    {
	SC_set_error(s.stmt, STMT_NO_MEMORY_ERROR,
...
RETCODE
SC_pos_refresh(StatementClass * stmt, SQLSETPOSIROW irow,
	       SQLULEN global_ridx)
{
    RETCODE ret;
    IRDFields *irdflds = SC_get_IRDF(stmt);
	...
	if (res && res->keyset)
	{
	    ...
	    if (kres_ridx >= 0
		&& kres_ridx < QR_get_num_cached_tuples(res))
	    {
		if (0 !=
		    (CURS_NEEDS_REREAD & res->keyset[kres_ridx].status))
		    tuple_reload = TRUE;
	    }
	}
    }
...
RETCODE spos_callback(RETCODE retcode, void *para)
{
    ...
    if (!res || !opts)
    {
	SC_set_error(s->stmt, STMT_SEQUENCE_ERROR,
		     "Passed res or opts for spos_callback is NULL",
		     func);
	return SQL_ERROR;
    }
	...
	s->ridx = s->nrow;
	pos_ridx = s->idx;
	if (0 != s->irow || !opts->row_operation_ptr
	    || opts->row_operation_ptr[s->nrow] == SQL_ROW_PROCEED)
	{
	    switch (s->fOption)
	    {
	    case SQL_UPDATE:
	    ...
	    {
		spos_cdata *cbdata =
		    (spos_cdata *) malloc(sizeof(spos_cdata));
	...
	*(SC_get_IRDF(s->stmt)->rowsFetched) = s->processed;
    ...
    if (opts)
    {
	inolog("processed=%d ret=%d rowset=%d", s->processed, ret,
	     ...
	     SQLUSMALLINT fOption, SQLUSMALLINT fLock)
{
    ...
    {
	SC_set_error(s.stmt, STMT_INVALID_CURSOR_STATE_ERROR,
		     "Null statement result in PGAPI_SetPos.", func);
	return SQL_ERROR;
    }

    rowsetSize =
	(s.stmt->transition_status ==
	 7 ? s.opts->size_of_rowset_odbc2 : s.opts->size_of_rowset);
    if (s.irow == 0)		/* bulk operation */
    {
	if (SQL_POSITION == s.fOption)
	{
	    SC_set_error(s.stmt, STMT_INVALID_CURSOR_POSITION,
			 "Bulk Position operations not allowed.", func);
	    return SQL_ERROR;
	}
	s.start_row = 0;
		       ...
		       SQLLEN crowKeyset, SQLUSMALLINT crowRowset)
{
    ...
    SC_set_error(stmt, STMT_NOT_IMPLEMENTED_ERROR,
		 "SetScroll option not implemeted", func);
...
PGAPI_SetCursorName(HSTMT hstmt,
		    const SQLCHAR FAR * szCursor, SQLSMALLINT cbCursor)
{
