...
#define WVFAIL(x) try { WvTest.test(!WvTest.booleanize(x), __FILE__, __LINE__, #x); } catch (NUnit.Framework.AssertionException _wvasserte) { throw _wvasserte; } catch (System.Exception _wvasserte) { WvTest.test_exception(__FILE__, __LINE__, #x); throw _wvasserte; }
