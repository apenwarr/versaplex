		     ...
		     SQLUSMALLINT fOption, SQLULEN vParam)
{
	...
	else if (stmt)
	{
		...
		stmt->options_orig.scroll_concurrency =
		(SQLUINTEGER) setval;
	}
	if (setval != vParam)
	    changed = TRUE;
	...
	{
	    if (0 !=
		(ci->updatable_cursors & ALLOW_KEYSET_DRIVEN_CURSORS))
		setval = vParam;
	    else
	...
	if (stmt && stmt->save_rowset_size <= 0
	    && stmt->last_fetch_count > 0)
	    stmt->save_rowset_size =
	...
	if (stmt)
	{
	    SC_set_error(stmt, STMT_NOT_IMPLEMENTED_ERROR,
			 "Simulated positioned update/delete not supported.  Use the cursor library.",
			 func);
	}
	if (conn)
	{
	...
	if (stmt)
	{
	    mylog("USE_BOOKMARKS %s\n",
		  (vParam ==
		   SQL_UB_OFF) ? "off" : ((vParam ==
					   SQL_UB_VARIABLE) ? "variable"
					  : "fixed"));
	    setval = vParam;
    ...
    default:
	{
	    ...
	    {
		SC_set_error(stmt, STMT_NOT_IMPLEMENTED_ERROR,
			     "Unknown statement option (Set)", NULL);
		sprintf(option, "fOption=%d, vParam=" FORMAT_LEN,
			fOption, vParam);
...
PGAPI_SetConnectOption(HDBC hdbc, SQLUSMALLINT fOption, SQLULEN vParam)
{
	...
	case SQL_TXN_SERIALIZABLE:
	    if (PG_VERSION_GE(conn, 6.5) && PG_VERSION_LE(conn, 7.0))
		retval = SQL_ERROR;
	    break;
	...
	{
	    CC_set_error(conn, CONN_INVALID_ARGUMENT_NO,
			 "Illegal parameter value for SQL_TXN_ISOLATION",
			 func);
	    return SQL_ERROR;
	} else
	{
	    ...
	    if (SQL_ERROR == retval)
	    {
		CC_set_error(conn, CONN_EXEC_ERROR,
			     "ISOLATION change request to the server error",
			     func);
		return SQL_ERROR;
	    }
	}
    ...
    default:
	{
	    ...
	    sprintf(option, "fOption=%d, vParam=" FORMAT_LEN, fOption,
		    vParam);
		...
		{
		    char *asPara =
			ucs2_to_utf8((SQLWCHAR *) vParam, SQL_NTS, NULL,
				     FALSE);
		       ...
		       SQLINTEGER BufferLength)
{
	...
	*((SQLUINTEGER *) pvParam) = (conn->status == CONN_NOT_CONNECTED
	...
	*((SQLUINTEGER *) pvParam) = CC_is_in_ansi_app(conn);
	...
	if (pvParam)
	{
	    ...
	    {
		len =
		    utf8_to_ucs2(p, len, (SQLWCHAR *) pvParam,
				 BufferLength / WCLEN);
		len *= WCLEN;
	    } else
#endif				/* UNICODE_SUPPORT */
		strncpy_null((char *) pvParam, p,
			     (size_t) BufferLength);
		...
		CC_set_error(conn, CONN_TRUNCATED,
			     "The buffer was too small for the pvParam.",
			     func);
	    }
	}
    }
...
RETCODE SQL_API
PGAPI_SetStmtOption(HSTMT hstmt, SQLUSMALLINT fOption, SQLULEN vParam)
{
		    ...
		    SQLINTEGER * StringLength, SQLINTEGER BufferLength)
{
	...
	{
	    SC_set_error(stmt, STMT_OPERATION_INVALID,
	...
	*((SQLINTEGER *) pvParam) = stmt->options.use_bookmarks;
    ...
    default:
	{
	    ...
	    SC_set_error(stmt, STMT_NOT_IMPLEMENTED_ERROR,
			 "Unknown statement option (Get)", NULL);
	    sprintf(option, "fOption=%d", fOption);
	    SC_log_error(func, option, stmt);
	    return SQL_ERROR;
	}
