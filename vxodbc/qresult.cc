...
char
QR_fetch_tuples(QResultClass * self, ConnectionClass * conn,
		const char *cursor)
{
	...
	mylog("%s: cursor = '%s', self->cursor=%p\n", func,
	      (cursor == NULL) ? "" : cursor, QR_get_cursor(self));
	...
	if (fetch_cursor)
	{
		...
		QR_set_message(self,
			       "Internal Error -- no cursor for fetch");
		return FALSE;
	    }
	}
	...
	self->count_backend_allocated = self->count_keyset_allocated =
	    0;
	if (self->num_fields > 0)
	{
	...
	{
	    if (self->keyset =
		(KeySet *) calloc(sizeof(KeySet), tuple_size),
		!self->keyset)
	    {
...
int QR_close(QResultClass * self)
{
	...
	if (CC_is_in_error_trans(conn))
	{
		...
		CC_mark_a_object_to_discard(conn, 'p',
					    QR_get_cursor(self));
	} else
	{
	    char buf[64];
...
int QR_next_tuple(QResultClass * self, StatementClass * stmt)
{
    ...
    inolog
	("in total_read=%d cursT=%d currT=%d ad=%d total=%d rowsetSize=%d\n",
	 self->num_total_read, self->cursTuple,
	 stmt ? stmt->currTuple : -1, self->ad_count,
	 QR_get_num_total_tuples(self),
	 self->rowset_size_include_ommitted);
    ...
    if (QR_once_reached_eof(self)
	&& self->cursTuple >= (Int4) QR_get_num_total_read(self))
	curr_eof = TRUE;
	...
	if (QR_is_moving_backward(self))
	{
	    ...
	    {
		SQLLEN incr_move =
		    self->cache_size - (req_size < 0 ? 1 : req_size);
	    ...
	    sprintf(movecmd, "move backward " FORMAT_ULEN " in \"%s\"",
		    movement, QR_get_cursor(self));
	} else if (QR_is_moving_forward(self))
	    sprintf(movecmd, "move " FORMAT_ULEN " in \"%s\"", movement,
		    QR_get_cursor(self));
	else
	{
	...
	if (sscanf(mres->command, "MOVE %lu", &moved) > 0)
	{
		...
		if (0 < moved)
		    moved++;
		else if (QR_is_moving_backward(self)
			 && self->cursTuple < 0)
		    ;
		...
		if (QR_is_moving_from_the_last(self))	/* in case of FETCH LAST */
		{
		    ...
		    SQLLEN rowset_start =
			self->cursTuple + 1, back_offset =
			self->move_offset, backpt;
		    inolog("FETCH LAST case\n");
		    if (getNthValid
			(self, QR_get_num_total_tuples(self) - 1,
			 SQL_FETCH_PRIOR, self->move_offset,
			 &backpt) < 0)
		    {
		    ...
		    back_offset =
			QR_get_num_total_tuples(self) - backpt;
			...
			sprintf(movecmd,
				"move backward " FORMAT_ULEN
				" in \"%s\"", bmovement,
				QR_get_cursor(self));
			QR_Destructor(mres);
			mres =
			    CC_send_query(conn, movecmd, NULL, 0, stmt);
			if (!QR_command_maybe_successful(mres))
			{
			    QR_Destructor(mres);
			    if (stmt)
			...
			if (sscanf(mres->command, "MOVE %lu", &mback) >
			    0)
			{
			    if (mback < bmovement)
				mback++;
			...
			QR_set_move_forward(self);
			self->move_offset =
			    moved + self->ad_count - back_offset - 1;
			rowset_start =
			    self->cursTuple + self->move_offset + 1;
			/* adjust move_offset */
						/*** self->move_offset++; ***/
		    }
		    if (stmt)
	...
	mylog
	    ("%s: fetch_number < fcount: returning tuple %d, fcount = %d\n",
	...
	{
	    mylog
		("%s: ALL_ROWS: done, fcount = %d, fetch_number = %d\n",
	...
	} else
	{
	    ...
	    {
		mylog
		    ("corrupted fetch_size end_tuple=%d <= cached_rows=%d\n",
	...
	if (enlargeKeyCache
	    (self, self->cache_size - num_backend_rows,
	     "Out of memory while reading tuples") < 0)
	    return FALSE;
	if (PROTOCOL_74(ci) && !QR_is_permanent(self)	/* Execute seems an invalid operation after COMMIT */
	    )
	{
	    ExecuteRequest = TRUE;
	...
	{
	    QResultClass *res;
	    sprintf(fetch, "fetch %d in \"%s\"", fetch_size,
		    QR_get_cursor(self));
    ...
    {
	mylog
	    ("%s: inTuples = true, falling through: fcount = %d, fetch_number = %d\n",
    ...
    curr_eof = reached_eof_now = (QR_once_reached_eof(self)
				  && self->cursTuple >=
				  (Int4) self->num_total_read);
	...
	case 'T':
		...
		/* set to first row */
		self->tupleField =
		    self->backend_tuples + (offset * num_fields);
	    } else
	    {
	    ...
	    if (!self->next)
	    {
		CC_set_error(conn, CONNECTION_COULD_NOT_RECEIVE,
			     "Could not create result info in send_query.",
			     func);
	...
	case 'C':		/* End of tuple list */
	    ...
	    mylog
		("end of tuple list -- setting inUse to false: this = %p %s\n",
		 self, cmdbuffer);
		...
		mylog
		    ("%s: backend_rows < CACHE_SIZE: brows = %d, cache_size = %d\n",
		     func, num_backend_rows, self->cache_size);
	    }
	    if (!internally_invoked || PG_VERSION_LE(conn, 6.3))
		rcvend = TRUE;
	    break;

	case 'E':		/* Error */
	    msg_truncated =
		handle_error_message(conn, msgbuffer, sizeof(msgbuffer),
				     self->sqlstate, "next_tuple",
				     self);
	...
	case 'N':		/* Notice */
	    msg_truncated =
		handle_notice_message(conn, cmdbuffer,
				      sizeof(cmdbuffer), self->sqlstate,
				      "next_tuple", self);
	...
	default:
	     ...
	     * dumped core ??? */
	    mylog
		("%s: Unexpected result from backend: id = '%c' (%d)\n",
		 func, id, id);
	    qlog("%s: Unexpected result from backend: id = '%c' (%d)\n",
		 func, id, id);
	    QR_set_message(self,
			   "Unexpected result from backend. It probably crashed");
	    QR_set_rstatus(self, PORES_FATAL_ERROR);
	    CC_on_abort(conn, CONN_DEAD);
	    ret = FALSE;
	    rcvend = TRUE;
	...
	if (reached_eof_now)
	{
		...
		{
		    start_idx =
			CacheIdx2GIdx(offset, stmt,
				      self) - self->num_total_read;
		...
		inolog
		    ("will add %d added_tuples from %d and select the %dth added tuple\n",
		     add_size, start_idx,
		     offset - num_backend_rows + start_idx);
		if (add_size > fetch_size - cur_fetch)
		    add_size = fetch_size - cur_fetch;
		else if (add_size < 0)
		    add_size = 0;
		if (enlargeKeyCache
		    (self, add_size,
		     "Out of memory while adding tuples") < 0)
		    return FALSE;
		/* append the KeySet info first */
		memcpy(self->keyset + num_backend_rows,
		       (void *) (self->added_keyset + start_idx),
		       sizeof(KeySet) * add_size);
		/* and append the tuples info */
		tuple =
		    self->backend_tuples +
		    num_fields * num_backend_rows;
		memset(tuple, 0,
		       sizeof(TupleField) * num_fields * add_size);
		added_tuple =
		    self->added_tuples + num_fields * start_idx;
		ReplaceCachedRows(tuple, added_tuple, num_fields,
				  add_size);
     ...
     */
    if (internally_invoked && self->keyset
	&& (self->dl_count > 0 || self->up_count > 0))
    {
	SQLLEN i, lf;
	SQLLEN lidx, hidx;
	...
	for (i = 0; i < self->dl_count && hidx > (Int4) deleted[i]; i++)
	{
		...
		self->keyset[lf].status =
		    self->deleted_keyset[i].status;
		/* mark the row off */
	...
	{
	    if (hidx > (Int4) updated[i] && lidx <= (Int4) updated[i])
	    {
		...
		ReplaceCachedRows(self->backend_tuples +
				  lf * num_fields,
				  self->updated_tuples + i * num_fields,
				  num_fields, 1);
		self->keyset[lf].status &= (~CURS_NEEDS_REREAD);
	    }
	}
