...
char CI_read_fields(ColumnInfoClass * self, ConnectionClass * conn)
{
    ...
    {
	SOCK_get_string(sock, new_field_name,
			2 * COLUMN_NAME_STORAGE_LEN);
	...
	mylog
	    ("%s: fieldname='%s', adtid=%d, adtsize=%d, atttypmod=%d (rel,att)=(%d,%d)\n",
	     func, new_field_name, new_adtid, new_adtsize,
	     new_atttypmod, new_relid, new_attid);

	if (self)
	    CI_set_field_info(self, lf, new_field_name, new_adtid,
			      new_adtsize, new_atttypmod, new_relid,
			      new_attid);
    }
