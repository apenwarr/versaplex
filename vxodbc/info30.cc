		...
		SQLSMALLINT FAR * pcbInfoValue)
{
	...
	len = 4;
	value =
	    SQL_SFKU_CASCADE | SQL_SFKU_NO_ACTION | SQL_SFKU_SET_DEFAULT
	    | SQL_SFKU_SET_NULL;
	break;
	...
	len = 4;
	value =
	    SQL_SVE_CASE | SQL_SVE_CAST | SQL_SVE_COALESCE |
	    SQL_SVE_NULLIF;
	break;
	...
	CC_set_error(conn, CONN_NOT_IMPLEMENTED_ERROR,
		     "Unrecognized key passed to SQLGetInfo30.", func);
	return SQL_ERROR;
    }
    result = SQL_SUCCESS;
	...
	if (rgbInfoValue)
	{
		...
		CC_set_error(conn, CONN_TRUNCATED,
			     "The buffer was too small for tthe InfoValue.",
			     func);
	    }
	}
#ifdef	UNICODE_SUPPORT
