...
ReportError(const char *errmsg, int line, const char *file)
{
    ...
    ret = SQLGetDiagRec(handletype, handle, 1, sqlstate, NULL, 
            msg, sizeof(msg), NULL);
    if (SQL_SUCCEEDED(ret))
    	log("SQL error %s -- %s (%s:%s)\n", 
            (char *)sqlstate, (char *)msg, file, line);
    else
    	log("SQLGetDiagRec failed (code %s) when attempting to retrieve "
    	    "error information\n", ret);
}
...
Connect(void)
{
    ...
    int res = SQLConnect(Connection, (SQLCHAR *) SERVER, SQL_NTS, 
