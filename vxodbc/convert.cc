 ...
 */
static BOOL
timestamp2stime(const char *str, SIMPLE_TIME * st, BOOL * bZone,
		int *zone)
{
    ...
    *zone = 0;
    ...
    bc[0] = '\0';
    if ((scnt =
	 sscanf(str, "%4d-%2d-%2d %2d:%2d:%2d%32s %16s", &st->y, &st->m,
		&st->d, &st->hh, &st->mm, &st->ss, rest, bc)) < 6)
	return FALSE;
    else if (scnt == 6)
	return TRUE;
    switch (rest[0])
...
static int ResolveOneParam(QueryBuild * qb, QueryParse * qp);
static int
...
static
RETCODE prep_params(StatementClass * stmt, QueryParse * qp,
		    QueryBuild * qb);
static int Prepare_and_convert(StatementClass * stmt, QueryParse * qp,
			       QueryBuild * qb)
{
...
#define	return	DONT_CALL_RETURN_FROM_HERE???
	...
	SC_scanQueryAndCountParams(orgquery, conn, &endp1, &num_p1,
				   NULL, NULL);
	srvquery = qb->query_statement;
	SC_scanQueryAndCountParams(srvquery, conn, &endp2, NULL, NULL,
				   NULL);
	mylog
	    ("%s:SendParseRequest for the first command length=%d(%d) num_p=%d\n",
	     func, endp2, endp1, num_p1);
	ret =
	    SendParseRequest(stmt, plan_name, srvquery, endp2, num_p1);
    } else
	ret =
	    SendParseRequest(stmt, plan_name, qb->query_statement,
			     SQL_NTS, -1);
    if (!ret)
	goto cleanup;
	...
	SC_set_error(stmt, STMT_EXEC_ERROR,
		     "commnication error while preapreand_describe",
		     func);
    ...
    {
	SC_set_error(stmt, STMT_EXEC_ERROR,
		     "Error while preparing parameters", func);
	goto cleanup;
    }
	...
	SC_scanQueryAndCountParams(orgquery, conn, &endp1, &num_p1,
				   &multi, NULL);
	SC_scanQueryAndCountParams(srvquery, conn, &endp2, &num_p2,
				   NULL, NULL);
	mylog
	    ("%s:SendParseRequest for the subsequent command length=%d(%d) num_p=%d\n",
	     func, endp2, endp1, num_p1);
	if (num_p2 > 0)
	{
	    stmt->current_exec_param = num_pa;
	    ret =
		SendParseRequest(stmt, plan_name, srvquery,
				 endp2 < 0 ? SQL_NTS : endp2, num_p1);
	    if (!ret)
		goto cleanup;
	    if (!SendDescribeRequest(stmt, plan_name))
		goto cleanup;
			       ...
			       BOOL buildPrepareStatement)
{
    ...
    if (!stmt->statement)
    {
	SC_set_error(stmt, STMT_INTERNAL_ERROR, "No statement string",
		     func);
	return SQL_ERROR;
    }
	...
	if (SC_is_fetchcursor(stmt))
	{
	    sprintf(new_statement, "%sdeclare \"%s\"%s cursor%s for ",
		    new_statement, SC_cursor_name(stmt), opt_scroll,
		    opt_hold);
	    qb->npos = strlen(new_statement);
	    qp->flags |= FLGP_CURSOR_CHECK_OK;
	    qp->declare_pos = qb->npos;
	} else if (SQL_CONCUR_READ_ONLY !=
		   stmt->options.scroll_concurrency)
	{
	...
	(0 != (qb->flags & FLGB_INACCURATE_RESULT));
	...
	conn->DriverToDataSource(conn->translation_option,
				 SQL_CHAR, new_statement,
				 (SDWORD) length, new_statement,
...
BOOL BuildBindRequest(StatementClass * stmt, const char *plan_name)
{
	...
	+ 1;
    if (QB_initialize
	(&qb, netleng > MIN_ALC_SIZE ? netleng : MIN_ALC_SIZE, stmt,
	 NULL) < 0)
    {
	return FALSE;
    }
	...
	for (i = stmt->proc_return, j = 0; i < num_params; i++)
	{
		...
		memcpy(bindreq + leng + sizeof(SWORD) * j, &net_one, sizeof(net_one));	/* binary */
	    }
	    j++;
	}
	...
	if (SQL_ERROR == SetStatementSvp(stmt))
	{
	    SC_set_error(stmt, STMT_INTERNAL_ERROR,
			 "internal savepoint error in SendBindRequest",
			 func);
	    ret = FALSE;
	    goto cleanup;
	}
...
static int ResolveOneParam(QueryBuild * qb, QueryParse * qp)
{
    ...
    int param_number;
    char param_string[128], tmp[256], cbuf[PG_NUMERIC_MAX_PRECISION * 2];	/* seems big enough to handle the data in this function */
    OID param_pgtype;
	...
	(0 ==
	 (qb->
	  flags & (FLGB_PRE_EXECUTING |
		   FLGB_BUILDING_PREPARE_STATEMENT)));
    ...
    inolog("resolveOneParam %d(%d,%d)\n", param_number,
	   ipdopts->allocated, apdopts->allocated);
    apara = NULL;
    ipara = NULL;
    ...
    inolog("ipara=%p paramType=%d %d proc_return=%d\n", ipara,
	   ipara ? ipara->paramType : -1, PG_VERSION_LT(conn, 8.1),
	   qb->proc_return);
	...
	{
	    qb->errormsg =
		"The function return value isn't marked as output parameter";
	    qb->errornumber = STMT_EXEC_ERROR;
	    CVT_TERMINATE(qb);	/* just in case */
	    return SQL_ERROR;
	}
	...
	{
	    qb->errormsg =
		"Output parameter isn't available before 8.1 version";
	...
	{
	    for (npos = qb->npos - 1;
		 npos >= 0 && isspace(qb->query_statement[npos]);
		 npos--);
	    if (npos >= 0)
	    {
		...
		case '(':
			...
			break;
		    for (npos = qp->opos + 1;
			 isspace(qp->statement[npos]); npos++);
	...
	if (current_row > 0)
	{
		...
		buffer += (bind_size * current_row);
		     ...
		     ctype_length(apara->CType), ctypelen > 0)
    ...
    if ((ipara && SQL_PARAM_OUTPUT == ipara->paramType)
	|| used == SQL_NULL_DATA || used == SQL_DEFAULT_PARAM)
    {
	if (req_bind)
	{
	    DWORD len = htonl(-1);
	...
	if (param_ctype == SQL_C_WCHAR && CC_default_is_c(conn))
	...
	mylog("C_WCHAR=%s(%d)\n", buffer, used);
	buf = allocbuf =
	    ucs2_to_utf8((SQLWCHAR *) buffer,
			 used > 0 ? used / WCLEN : used, &used, FALSE);
	/* used *= WCLEN; */
	break;
#endif				/* UNICODE_SUPPORT */
    ...
    case SQL_C_SLONG:
    case SQL_C_LONG:
	sprintf(param_string, FORMAT_INTEGER, *((SQLINTEGER *) buffer));
	break;

    case SQL_C_SBIGINT:
    case SQL_BIGINT:		/* Is this needed ? */
	sprintf(param_string, FORMATI64, *((SQLBIGINT *) buffer));
	break;

    case SQL_C_UBIGINT:
	sprintf(param_string, FORMATI64U, *((SQLUBIGINT *) buffer));
	break;

    case SQL_C_SSHORT:
    case SQL_C_SHORT:
	sprintf(param_string, "%d", *((SQLSMALLINT *) buffer));
	break;

    case SQL_C_STINYINT:
    case SQL_C_TINYINT:
	sprintf(param_string, "%d", *((SCHAR *) buffer));
	break;
    ...
    case SQL_C_USHORT:
	sprintf(param_string, "%u", *((SQLUSMALLINT *) buffer));
	break;

    case SQL_C_UTINYINT:
	sprintf(param_string, "%u", *((UCHAR *) buffer));
	break;
    ...
    case SQL_C_TYPE_TIMESTAMP:	/* 93 */
	{
	    ...
	    mylog("m=%d,d=%d,y=%d,hh=%d,mm=%d,ss=%d\n", st.m, st.d,
		  st.y, st.hh, st.mm, st.ss);

	    break;

	}
    case SQL_C_NUMERIC:
	if (ResolveNumericParam
	    ((SQL_NUMERIC_STRUCT *) buffer, param_string))
	    break;
    default:
	/* error */
	qb->errormsg =
    ...
    if (conn->connInfo.cvt_null_date_string > 0
	&& PG_TYPE_DATE == param_pgtype && SQL_C_CHAR == param_ctype
	&& NULL != buf && '\0' == buf[0])
    {
	if (req_bind)
	{
