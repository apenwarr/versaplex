...
static BOOL out_check(void *out, size_t len, const char *name)
{
	...
	if ((UInt8) out < (UInt8) (altbl[i].aladr) + altbl[i].len)
	{
		...
		ret = FALSE;
		mylog
		    ("%s:%s:out_check found memory buffer overrun %p(%d)>=%p(%d)\n",
		     ALCERR, name, out, len, altbl[i].aladr,
		     altbl[i].len);
	    }
	    break;
	}
...
void debug_memory_check(void)
{
    ...
    {
	mylog
	    ("no memry leak found and max count allocated so far is %d\n",
	     alsize);
	free(altbl);
	alsize = 0;
    } else
