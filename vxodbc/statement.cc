 ...
 */
void
SC_scanQueryAndCountParams(const char *query,
...
#define	return	DONT_CALL_RETURN_FROM_HERE???
	...
	issue_begin = FALSE;
    else if (CC_is_in_autocommit(conn) && (!SC_is_fetchcursor(self)
	...
	{
	    SC_set_error(self, STMT_EXEC_ERROR,
			 "Could not begin a transaction", func);
	    goto cleanup;
	}
    }
    ...
    {
	SC_set_error(self, STMT_OPERATION_CANCELLED,
		     "Cancel Reuest Accepted", func);
	goto cleanup;
    }
    ...
    if (PREPARED_PERMANENTLY == self->prepared && PROTOCOL_74(ci))
	...
	if (!(res = SendSyncAndReceive(self, res, "bind_and_execute")))
	{
	    if (SC_get_errornumber(self) <= 0)
		SC_set_error(self, STMT_EXEC_ERROR,
			     "Could not receive he response, communication down ??",
			     func);
	...
	qflag |=
	    (SQL_CONCUR_READ_ONLY !=
	     self->options.scroll_concurrency ? CREATE_KEYSET : 0);

	mylog
	    ("       Sending SELECT statement on stmt=%p, cursor_name='%s' qflag=%d,%d\n",
	     self, SC_cursor_name(self), qflag,
	     self->options.scroll_concurrency);

	/* send the declare/select */
	res =
	    CC_send_query(conn, self->stmt_with_params, NULL, qflag,
			  SC_get_ancestor(self));
	    ...
	    && QR_command_maybe_successful(res))
	{
	     ...
	     * cache accordingly.
	     */
	    sprintf(fetch, "fetch " FORMAT_LEN " in \"%s\"",
		    qi.row_size, SC_cursor_name(self));

	    res =
		CC_send_query(conn, fetch, &qi, qflag,
			      SC_get_ancestor(self));
	...
	if (was_ok)
	    ...
	    SC_set_errornumber(self,
			       was_nonfatal ? STMT_INFO_ONLY :
	...
	} else
	{
	    ...
	    for (tres = res, numcols = 0; !numcols && tres;
		 tres = tres->next)
	    {
		...
		if (opts->bindings == NULL)
		{
		    ...
		    SC_set_error(self, STMT_NO_MEMORY_ERROR,
				 "Could not get enough free memory to store the binding information",
				 func);
		    goto cleanup;
		}
	    }
	...
	{
	    SC_set_error(self, STMT_CREATE_TABLE_ERROR,
			 "Error creating the table", func);
	...
	if (SQL_SUCCEEDED(ret))
	{
	    ...
	    SQLULEN offset =
		apdopts->param_offset_ptr ? *apdopts->
		param_offset_ptr : 0;
		    ...
		    ipara->paramType == SQL_PARAM_INPUT_OUTPUT)
		{
		    ...
		    ret =
			PGAPI_GetData(hstmt, gidx + 1, apara->CType,
				      apara->buffer + offset,
				      apara->buflen,
				      apara->
				      used ? LENADDR_SHIFT(apara->used,
							   offset) :
				      NULL);
		    if (!SQL_SUCCEEDED(ret))
		    {
	...
	{
	    SC_set_error(self, STMT_EXEC_ERROR,
			 "SC_fetch to get a Procedure return failed.",
			 func);
	}
    }
  cleanup:
...
BOOL SendBindRequest(StatementClass * stmt, const char *plan_name)
{
				 ...
				 const char *comment)
{
	...
	{
	    SC_set_error(stmt, CONNECTION_NO_RESPONSE,
			 "No response rom the backend", func);
	...
	case 'C':
		...
		} else
		{
		    ...
		    ret1 = ret2 = 0;
		    if (sscanf(msgbuffer, "%*s %d %d", &ret1, &ret2) >
			1)
	...
	case 'E':		/* ErrorMessage */
		...
		handle_error_message(conn, msgbuffer, sizeof(msgbuffer),
				     res->sqlstate, comment, res);

	    rcvend = TRUE;
	    break;
	case 'N':		/* Notice */
	    msg_truncated =
		handle_notice_message(conn, msgbuffer,
				      sizeof(msgbuffer), res->sqlstate,
				      comment, res);
	    break;
	...
	case 't':		/* ParameterDesription */
	    ...
	    {
		mylog
		    ("ParamInfo unmatch num_params(=%d) != info(=%d)+discard(=%d)\n",
	    ...
	    if (stmt->discard_output_params)
	    {
		for (i = 0, pidx = stmt->proc_return;
		     i < num_p && pidx < stmt->num_params; pidx++)
		{
	    ...
	    {
		for (i = 0, pidx = stmt->proc_return; i < num_p;
		     i++, pidx++)
		{
		    paramType = ipdopts->parameters[pidx].paramType;
		    oid = SOCK_get_int(sock, 4);
		...
		{
		    mylog
			("%dth parameter's position(%d) is out of bound[%d]\n",
	...
	case 'T':		/* RowDesription */
		...
		num_io_params =
		    CountParameters(stmt, NULL, &dummy1, &dummy2);
		if (stmt->proc_return > 0 || num_io_params > 0)
		{
		    ipdopts = SC_get_IPDF(stmt);
		    extend_iparameter_bindings(ipdopts,
					       stmt->num_params);
			...
			{
			    inolog("!![%d].PGType %u->%u\n", i,
				   ipdopts->parameters[i].PGType,
				   CI_get_oid(res->fields, cidx));
			    ipdopts->parameters[i].PGType =
				CI_get_oid(res->fields, cidx);
			    cidx++;
			}
...
BOOL
SendParseRequest(StatementClass * stmt, const char *plan_name,
		 const char *query, Int4 qlen, Int2 num_params)
{
	...
	CC_set_error(conn, CONNECTION_COULD_NOT_SEND,
		     "Could not send P request to backend", func);
	CC_on_abort(conn, CONN_DEAD);
	return FALSE;
    }
