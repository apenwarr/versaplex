...
@"Usage: sm [--dry-run] [--force] [--verbose] <command> <moniker> <dir>
  ...
  Valid commands: push, pull, dpush, dpull, pascalgen, csgen

  pascalgen usage:
    sm [--verbose] [--global-syms=<syms>] pascalgen <classname> <dir>
  csgen usage:
    sm [--verbose] csgen <namespace> <dir>
...
");
    ...
    static void do_csgen(string nsname, string dir, string outfile)
    {
	Console.Error.Write("Generating C# wrapper file...\n");
	...
	var tables = new List<string>();
	    ...
	    sb.Append(wv.fmt("    public class {0}\n    {{\n", elem.name));
	    
	    foreach (VxSchemaTableElement col in (VxSchemaTable)elem)
	    {
		if (col.elemtype == "column")
		{
		    bool autoincr = col.GetParam("identity_seed").ne();
		    string typ = col.GetParam("type");
		    string name = col.GetParam("name");
		    
		    // FIXME: this is a cheesily limited mapping of database
		    // types to C# types...
		    if (typ.StartsWith("int"))
			typ = "int";
		    else if (typ.StartsWith("date"))
			typ = "DateTime";
		    else
			typ = "string";
		    
		    if (autoincr)
			sb.Append(wv.fmt("        [PrimaryKeyColumn] " +
					 "public {0} {1};\n",
					 typ, name));
		    else
			sb.Append(wv.fmt("        [Column] " +
					 "public {0} {1};\n",
					 typ, name));
	...
	sb.Append(
@"    public class Database : Wv.Query.Database
    {
        public Database(WvDbi dbi) : base(dbi) { }
        public Database(string dbi_moniker) : base(dbi_moniker) { }

");
	foreach (var t in tables)
	    sb.Append(wv.fmt
		      ("        public QuerySet<{0}> {0} {{\n" +
		       "            get {{ return new QuerySet<{0}>(dbi); }}\n" +
		       "        }}\n", t));
	sb.Append("    }\n");
    ...
    public static int csgen_Main(List<string> extra,
				 string outfile)
    {
	if (extra.Count != 3)
	{
	...
	string nsname = extra[1];
	string dir    = extra[2];
	
	do_csgen(nsname, dir, outfile);
	return 0;
    }
	...
	if (cmd == "csgen")
	    return csgen_Main(extra, outfile);
